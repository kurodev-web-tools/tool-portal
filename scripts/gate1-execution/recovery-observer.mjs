...
import { http, lookup, publicHttp } from './observer-network.mjs';
...
function publishedSql(){
 ...
 const match=source.match(/^const SQL = ("(?:[^"\\]|\\.)*");$/m);assert.ok(match);const sql=JSON.parse(match[1]);assert.ok(sql.includes("'public.comment_translator_paid_entitlements'"));return sql.replace("'public.comment_translator_paid_entitlements'","'public.ct_gate1_recovery_probe'");
}
...
async function main(){
 ...
 const JOURNAL=preflightOnly?ROOT+'/recovery-preflight.jsonl':LIVE_JOURNAL;
 const input=JSON.parse(text);assert.ok(exact(input,['projectRef','key','stopEvidencePolicy','bridgeDirectory','runId']));
 assert.equal(input.stopEvidencePolicy,STOP_EVIDENCE_POLICY);assert.equal(input.bridgeDirectory,ROOT+(preflightOnly?'/recovery-preflight-management-bridge':'/recovery-management-bridge'));
 ...
 const saved=nativeBinding('recovery'),binding=saved.binding;
 assert.equal(binding.target,'recovery');assert.equal(input.projectRef,binding.projectRef);assert.match(input.key,/^sb_publishable_[A-Za-z0-9_-]{10,200}$/);
 ...
 const emit=value=>{
  const row={...policy,target:'recovery',observerSha256,bridgeSha256,observedAt:new Date().toISOString(),...value};
 ...
 try{
  ...
  const password=fs.readFileSync(saved.credentialFile,'utf8').replace(/^\uFEFF/u,'').replace(/\r?\n$/u,'');assert.ok(password&&!/[\r\n\0]/.test(password));
  ...
  while(elapsed()<=1200000){
   ...
   const c=readObserverControl(ROOT,'recovery',runId);
   ...
   console.log(JSON.stringify({...policy,target:'recovery',observerSha256,stage:'PAIR_TERMINAL',stoppingEvidence:evidence,stopEvidence:!!evidence,restoreEligible:false,receiptSha256:sha(journalBytes),receiptBytes:journalBytes.length,elapsedMs:Math.ceil(elapsed()),mutations:0}));
...
if(process.argv[1]&&fileURLToPath(import.meta.url)===path.resolve(process.argv[1]))main().catch(()=>{console.log(JSON.stringify({target:'recovery',stage:'OBSERVER_FAILED',phase:safePhase,stopEvidence:false,mutations:0}));process.exitCode=1;});
