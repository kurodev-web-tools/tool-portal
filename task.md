...
- Current branch: `codex/comment-translator-free-beta-retention-attribution`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- This branch is F13 Data deletion, retention, and source attribution for Free beta only unless the release owner explicitly expands scope.
...
1. F13 Data deletion, retention, and source attribution
   - Goal: make Free public beta deletion request state, OAuth disconnect cleanup readiness, retention readiness, deleted-message propagation, and `Source: YouTube Live Chat` attribution browser-safe and deterministic from existing server-owned durable session state, durable usage counter state, public entitlement baseline, live feed normalization, and provider readiness boundaries.
   - Scope: server-only local retention/source resolver, sanitized data deletion server action path, OAuth disconnect cleanup readiness marker, message-reference-only deleted-message tombstone propagation, source attribution labels on server-owned live comment surfaces, focused contract, docs, and `task.md`. No real OAuth live disconnect execution, provider target lookup, live target lookup, `liveChatMessages.list`, session start smoke, Azure/OpenAI provider API execution, live/provider execution, remote mutation/schema apply, deploy/upload, Stripe live action, main promotion, public launch gate flip, browser storage expansion, handoff payload expansion, manual live target entry, route-render lookup, or connection-only monitoring is approved or run in this thread.
   - Status: complete in this PR.
   - Implemented:
     - `lib/comment-translator-free-beta-retention-attribution.ts` adds a server-only Free beta retention/source resolver for deletion request availability, retention job readiness, OAuth disconnect cleanup readiness, deleted-message propagation, source attribution, and sanitized unavailable states.
     - `app/tools/comment-translator/actions.ts` exposes `requestCommentTranslatorDataDeletionAction()` as the deletion button path. It derives readiness from durable session/usage reads, Free entitlement baseline, and provider readiness, and returns sanitized unavailable state rather than performing cleanup mutation when prerequisites are unreadable.
     - `lib/comment-translator-live-message-normalization.ts`, `lib/comment-translator-real-comments-ui-wiring.ts`, and `lib/comment-translator-real-comments-feed-shared.ts` carry `Source: YouTube Live Chat` and message-reference tombstone metadata without raw provider payloads, raw comments, author channel material, provider target metadata, or server-only cursors.
     - `components/comment-translator/CommentTranslatorDock.tsx` and `lib/comment-translator.ts` add a compact deletion/retention/source panel plus source badges on server-owned live comment cards only.
     - `docs/active/COMMENT_TRANSLATOR_DURABLE_PERSISTENCE_SCHEMA_READINESS.md` and `docs/active/COMMENT_TRANSLATOR_PUBLIC_BETA_GAP_AUDIT.md` record F13 as local deterministic deletion/retention/source attribution wiring with real cleanup/live/remote execution still approval-gated.
   - Output/privacy boundary: F13 output keeps raw provider payload, raw comments, author channel id/URL/profile image URL, owner user id, provider channel id, live target value, provider target metadata, liveChatId, service_role, Authorization header, token values, provider error body, server-only cursor, browser storage, and handoff payload material out of returned session/UI/docs/browser-readable state. Browser-readable additions are limited to sanitized deletion/retention/source labels, unavailable reasons, and tombstone state.
   - Remote/live execution: OAuth live disconnect execution, cleanup mutation, provider target lookup execution, live target lookup execution, real `liveChatMessages.list`, real provider payload capture, session start smoke, Azure/OpenAI provider API execution, live/provider execution, remote Supabase migration apply, deploy/upload, Stripe live action, main promotion, and public launch gate flip are all not-run/approval-gated.
   - F13 verification: `node scripts/comment-translator-free-beta-retention-attribution-contract.mjs`, changed-files no-secret scan over 11 changed non-contract files, `npm run lint`, `npx tsc --noEmit`, `npm run build`, and `git diff --check` all exit 0. `npm run build` emitted the existing Next middleware/proxy deprecation warning plus webpack cache serialization warnings; `git diff --check` emitted Windows line-ending warnings only.
   - Width checks: Codex app in-app browser against local `next dev` on `127.0.0.1:3000` rendered `/tools/comment-translator/` at `390 / 820 / 1024 / 1280 / 1366px`; local unauthenticated/env-local state reached the private-launch fallback surface at each width with meaningful content, console error/warn count `0`, framework overlay absent, document horizontal overflow `false`, body horizontal overflow `false`, and no detected overflowing elements.
   - Width-check limitation: the authenticated allowed-tester translator dock, F13 deletion/retention/source panel, and live comment cards with `Source: YouTube Live Chat` could not be rendered locally because no safe local Supabase/auth/allowed-tester session was available. The F13 focused contract covers the panel metadata/copy path, deletion server action path, source label, and deleted-message tombstone propagation; live/session UI evidence remains unchecked until an approved allowed-tester route check is available.
   - Residual risk / unchecked scope: real OAuth disconnect cleanup mutation, real Azure/OpenAI provider API execution, real YouTube provider target lookup/live target resolution, actual `liveChatMessages.list`, non-empty live comment intake, authenticated allowed-tester translator dock rendering with live session state, deployed durable usage/session enforcement, remote schema application, route/API smoke with authenticated allowed tester, and launch readiness remain unverified until same-thread ready preflight, sanitized output review, and explicit in-thread approval exist. Public release remains blocked until F14-F15 and approved live/remote evidence are complete.
   - Next safe action: after this PR merges, start F14 Creator locked cards / waitlist / click tracking in a separate branch/PR from latest `origin/codex/comment-translator-free-public-beta-integration`.
...
2. F11 Start/Stop reason UX
   ...
   - Status: complete in this PR.
   - Implemented:
     - `lib/comment-translator-start-stop-reason-ux.ts` adds a browser-safe reason resolver for disconnected, reconnect-required, auth unavailable, no live broadcast, live chat disabled, ended/not found, quota/budget stop, heartbeat/browser disconnect, translation/provider unavailable/error, session limit, and user stop states.
     - `lib/comment-translator-session-runtime.ts`, `lib/comment-translator-server-only-live-chat-target-lookup.ts`, and `lib/comment-translator-bounded-live-chat-polling-wiring.ts` carry sanitized reason codes from existing readiness/polling/session boundaries into stopped session state without raw provider payloads or target metadata.
     - `components/comment-translator/CommentTranslatorDock.tsx` and `lib/comment-translator.ts` show localized reason group/message/recommended action in the existing session panel and details row.
     - `docs/active/COMMENT_TRANSLATOR_DURABLE_PERSISTENCE_SCHEMA_READINESS.md` and `docs/active/COMMENT_TRANSLATOR_PUBLIC_BETA_GAP_AUDIT.md` record F11 as local deterministic UX wiring with real provider/API execution still approval-gated.
   - Output/privacy boundary: F11 output keeps raw provider payload, raw comments, author channel id/URL/profile image URL, owner user id, provider channel id, live target value, provider target metadata, liveChatId, service_role, Authorization header, token values, provider error body, server-only cursor, browser storage, and handoff payload material out of returned session/UI/docs/browser-readable state. Browser-readable additions are limited to sanitized reason code/group/action metadata plus localized static copy.
   ...
   - F11 verification: `node scripts/comment-translator-start-stop-reason-ux-contract.mjs`, changed-files no-secret scan over 14 changed files with contract scripts skipped for synthetic forbidden fixtures, `npm run lint`, `npx tsc --noEmit`, `npm run build`, and `git diff --check` all exit 0. `npm run build` emitted the existing Next middleware/proxy warning; `git diff --check` emitted Windows line-ending warnings only.
   ...
   - Width-check limitation: the authenticated allowed-tester translator dock and the new conditional `reasonUx` panel could not be rendered locally because no safe local Supabase/auth/allowed-tester session was available. The F11 focused contract covers the reason panel metadata/copy path; live/session UI evidence remains unchecked until an approved allowed-tester route check is available.
   - Residual risk / unchecked scope: real Azure/OpenAI provider API execution, real YouTube provider target lookup/live target resolution, actual `liveChatMessages.list`, non-empty live comment intake, authenticated allowed-tester translator dock rendering with live session state, deployed durable usage/session enforcement, remote schema application, route/API smoke with authenticated allowed tester, and launch readiness remain unverified until same-thread ready preflight, sanitized output review, and explicit in-thread approval exist. Public release remains blocked until F12-F15 and approved live/remote evidence are complete.

2. F10 Azure normal translation execution
