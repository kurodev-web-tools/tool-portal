...
PL-G2K allowed-tester route/API harness smoke execution: `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G2K_APPROVED_ALLOWED_TESTER_ROUTE_API_HARNESS_SMOKE_EXECUTION_AFTER_PL_G2J.md` rechecks the PL-G2 status route and PL-G2A harness route after PL-G2J with exact approval label present and records keep blocked / blocked-missing-operator-local-reference-readiness. The allowed-tester route/API harness smoke remains blocked-missing-operator-local-reference-readiness / not-run / approval-gated because deployed origin reference ready and allowed-tester cookie/session reference ready are present as value-free confirmations, but harness env gate reference ready is missing in-thread.
