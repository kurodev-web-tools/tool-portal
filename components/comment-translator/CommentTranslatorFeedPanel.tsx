...
export function CommentTranslatorFeedPanel({ locale, copy, commentOnly, sessionStatus, authorDisplayNamePolicy, searchQuery, statusFilter, statusFilters, priorityFilter, priorityFilteredCommentCount, filteredComments, publicCommentCount, translatedCount, displayMode, targetLanguageLabel, isPending, hasRetainedRows, errorMessage, unavailableMessage, onNormalView, onSearchQueryChange, onStatusFilterChange, onPriorityFilterChange, onRefresh, onClear }: {
  ...
  readonly priorityFilter: CommentTranslatorPriorityFilter;
  readonly priorityFilteredCommentCount: number;
  ...
  readonly onPriorityFilterChange: (value: unknown) => void;
  readonly onRefresh: () => void;
  readonly onClear: () => void;
}) {
  const showPriorityEmpty = priorityFilter === "priority" && priorityFilteredCommentCount === 0 && !errorMessage && !unavailableMessage;
  return (
    <main data-comment-translator-real-comments-feed="server-owned-safe-rows" data-comment-translator-obs-dock-display-name-policy={authorDisplayNamePolicy.marker} className="panel flex min-h-[34rem] min-w-0 flex-col overflow-hidden">
      ...
      <div className="scrollbar-accent min-h-0 flex-1 space-y-3 overflow-auto p-3 sm:p-4">{filteredComments.length > 0 ? filteredComments.map((comment) => <CommentTranslatorCommentCard key={comment.id} comment={comment} displayMode={displayMode} authorDisplayNamePolicy={authorDisplayNamePolicy} targetLanguageLabel={targetLanguageLabel} copy={copy} />) : <div className="grid min-h-72 place-items-center rounded-base border border-dashed border-border bg-background/70 p-4 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-base bg-surface-muted text-2xl text-muted">...</div><p className="mt-3 text-sm font-black text-foreground">{showPriorityEmpty ? locale === "ja" ? "優先コメントはありません" : "No priority comments" : locale === "ja" ? <><span className="inline-block">一致するコメントは</span><span className="inline-block">ありません</span></> : copy.empty.title}</p><p className="mt-1 text-xs leading-5 text-muted">{errorMessage ?? unavailableMessage ?? (showPriorityEmpty ? locale === "ja" ? "現在表示できる優先コメントはありません。" : "No priority comments are currently available." : copy.empty.body)}</p></div></div>}</div>
    </main>
  );
}
