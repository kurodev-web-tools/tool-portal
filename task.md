...
1. Kuro Live Comment Translator public release roadmap
   - status: preview runtime-smoke-to-operator-UI chain is complete through Task 7, Public Release Roadmap Task 1-15 are merged, and Task 16 final QA/readiness PR #419 is merged. PR #419 merge commit `2b11066de57bcd75507930525fde8b8d143681aa` is contained in `origin/codex/comment-translator-preview`. GitHub PR metadata was also checked: PR #419 is `MERGED`, base `codex/comment-translator-preview`, head `codex/comment-translator-public-release-final-qa-post-pr418`, merged at `2026-06-11T12:57:20Z`; check rollup was Cloudflare Pages `FAILURE`, treated as the known non-main Pages build posture rather than a fresh local regression by itself.
   - current PR scope: Pre-Main Launch Hardening Roadmap Task 17, Private launch access gate.
   ...
   - inspected surfaces for Task 17: `task.md`, `docs/active/COMMENT_TRANSLATOR_PUBLIC_RELEASE_FINAL_QA.md`, `docs/active/COMMENT_TRANSLATOR_PUBLIC_RELEASE_REQUIREMENTS.md`, `app/tools/comment-translator`, `app/account`, `app/api/comment-translator`, `lib/comment-translator-*`, auth/session helpers, billing runtime/actions, YouTube credential status/disconnect boundaries, and existing comment-translator contract scripts.
   - completed in this branch: added `lib/comment-translator-private-launch-access-gate.ts` as a server-only, fail-closed private launch gate using `COMMENT_TRANSLATOR_PRIVATE_LAUNCH_ALLOWED_USER_HASHES` with SHA-256 owner-user-id hashes only. `/tools/comment-translator`, `/account/integrations`, and `/account/billing` now return a disabled/coming-soon private launch state for non-allowed users, while allowed testers can reach the existing preview flow. Comment-translator session, credential status, disconnect APIs/actions, account integration actions, and billing checkout/portal actions now fail closed before provider/session/billing-affecting work for non-allowed users.
   ...
   - verification for this branch: RED check for `node scripts/comment-translator-private-launch-access-gate-contract.mjs` failed before implementation because `lib/comment-translator-private-launch-access-gate.ts` was missing; GREEN passed after implementation. `npm run lint`, `npx tsc --noEmit`, `npm run build`, `git diff --check`, changed-file no-secret scan, and relevant-surface no-secret scan passed. `npm run build` retained existing non-blocking warnings about deprecated `middleware`, skipped static-export RSC aliases for server-runtime build, and webpack cache serialization.
   - route/API negative checks: local dev server `http://127.0.0.1:3217` direct calls as a non-allowed/unauthenticated context returned 403 for `POST /api/comment-translator/session`, `GET /api/comment-translator/youtube/credential-status`, and `POST /api/comment-translator/youtube/disconnect`; bodies were sanitized stopped/unavailable states with no token, owner id, provider target, liveChatId, Stripe secret, or Authorization header values.
   - allowed-tester checks: focused contract verified matching SHA-256 owner-user-id hash in `COMMENT_TRANSLATOR_PRIVATE_LAUNCH_ALLOWED_USER_HASHES` returns `allowed-tester`, raw owner-user-id strings in that env are ignored, and allowed output excludes the raw owner id value. Live account/browser tester execution was not run because this thread has no approval for provider/live execution and no private identifiers were requested.
   - width verification: Codex in-app Browser checks for `/tools/comment-translator`, `/account/integrations`, and `/account/billing` at `390 / 820 / 1024 / 1280 / 1366px` found no horizontal overflow, no framework overlay, and no browser console warn/error logs. Each route rendered the private launch coming-soon/disabled state. Link interaction from `/tools/comment-translator` to `/tools/` was verified.
   - public-release capable: no. Private launch gate is now implemented for main/production promotion safety, but production/custom deployed smoke, live/provider smoke, Stripe live-mode action, provider/cost policy finalization, security review, and final public launch gate flip remain incomplete or approval-gated.
   - residual risk: real allowed-tester production verification still requires operator-local configuration of `COMMENT_TRANSLATOR_PRIVATE_LAUNCH_ALLOWED_USER_HASHES` and later private-gated production/custom smoke. Live/provider execution and Stripe live-mode actions remain separately approval-gated.
   - next safe action: create this Task 17 implementation PR targeting `codex/comment-translator-preview`, then proceed to Task 18 Operator UX readiness polish as a separate PR after merge.
