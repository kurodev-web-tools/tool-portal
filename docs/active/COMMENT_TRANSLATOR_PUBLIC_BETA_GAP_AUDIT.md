...
| Stop reason UX and usage display | F11 local deterministic Start/Stop reason UX maps existing server-owned session stop reasons, credential readiness, live-target readiness, polling/provider signal, quota/budget state, and heartbeat/browser state to sanitized reason metadata and localized copy. UI still uses mock-like usage display until F12. | implemented local foundation / gated | Real provider lookup, polling, translation API execution, deployed durable evidence, and F12 usage display remain required before public beta. |
