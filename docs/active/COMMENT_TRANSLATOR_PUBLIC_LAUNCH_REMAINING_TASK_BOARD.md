...
The PL-G6 public access change preflight records the exact same-thread approval surface and keeps `public_release_capable=no`, `pl_g6_public_access_change_status=not-run-approval-gated`, public gate flip, public access change, and main promotion not-run. The PL-G6C update now records operator-provided production/main-domain env readiness plus private-launch-only production smoke evidence without exposing values or raw comments.
...
- Public launch operator QA checklist: complete for docs/contract separation and 2026-07-09 / 2026-07-10 sanitized operator updates. Preview Managed Challenge setup, safe edge-control reference presence, login-only Free beta browser smoke, Creator/paid waitlist boundary smoke, YouTube-connect no-autostart smoke, production harness blocking, and production/main-domain private-launch Start-to-translation smoke are recorded as partial external pass. Production edge activation, optional burst comment smoke, optional 30-minute session smoke, optional monthly cap fixture/live smoke, public gate flip, remote mutation, public access change, and main promotion remain not-run / approval-gated.
...
- PL-G6 public access change preflight: complete for docs/contract guidance and sanitized evidence recording. The preflight doc is `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G6_PUBLIC_ACCESS_CHANGE_PREFLIGHT.md`; it records exact same-thread approval text and operation boundaries while keeping `public_release_capable=no`, `pl_g6_public_access_change_status=not-run-approval-gated`, public gate flip, public access change, and main promotion not-run. PL-G6C now records sanitized operator instructions for production/main-domain env readiness and operator-provided private-launch-only production smoke evidence. No Cloudflare mutation, public gate flip, public access change, Supabase query/mutation/migration, Stripe live action, paid entitlement runtime, OBS overlay route/token runtime, or main promotion was run in this evidence-record slice.
