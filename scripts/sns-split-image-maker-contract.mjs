...
const copySourcePath = path.join(root, "lib", "sns-split-image-maker-copy.ts");
...
assert.match(copySource, /concatenate: "個別追加"/, "UI labels include individual-add mode");
assert.match(copySource, /replace: "フレーム追加"/, "UI labels include frame-add mode");
assert.match(copySource, /post: "完成形"/, "preview tabs use a short final-image label");
...
assert.match(copySource, /メイン画像を選択してから出力してください。/, "manual export call warns before exporting without a main image");
assert.match(copySource, /メイン画像を選ぶと、投稿順プレビューとPNG\/JPEG保存が有効になります。/, "empty main-image state explains why export is disabled");
assert.match(copySource, /Thumbnail Editorからの画像が見つからなかったため、メイン画像は未選択のまま開始しました。/, "failed thumbnail handoff has a concrete recovery message");
assert.match(copySource, /投稿順プレビュー/, "preview tab names the posting-order preview explicitly");
assert.match(copySource, /split_1から保存順に確認/, "posting-order preview explains the save order");
assert.match(copySource, /完成形プレビューです。メイン分割を1枚の画像として確認/, "final-image preview explains the main split as one complete image");
...
assert.match(copySource, /境界を動かすときは枠線を表示したまま/, "boundary adjustment guidance stays visible near preview controls");
assert.match(copySource, /投稿別調整はドラッグまたはスライダーで行います。/, "post adjustment guidance explains drag and slider controls");
assert.match(copySource, /選択中の形式で個別ファイル保存/, "export section describes single-format individual file output");
assert.match(appSource, /getExportButtonLabel/, "export button label is generated from current PNG/JPEG format and post count");
assert.match(copySource, /の順で\$\{postCount\}枚を書き出しました。/, "successful export toast includes output order and count");
assert.match(copySource, /PNG\/JPEGはどちらか1形式を選び、ZIPや複数形式の一括出力は後続候補です。/, "export copy keeps zip and multi-format batch out of current features");
...
assert.match(copySource, /Thumbnail Editorから画像を受け取りました。/, "thumbnail handoff copy says the base image was received");
assert.match(copySource, /Schedule Calendarから告知文メモを受け取りました。/, "schedule handoff copy says only text memo was received");
assert.match(copySource, /受け取った画像をメイン画像として確認し、必要なら追加画像を入れてから保存します。/, "thumbnail handoff next action stays concrete");
assert.match(copySource, /メイン画像は未選択です。画像を選んでから、告知文メモを投稿文へ使えます。/, "schedule handoff next action explains the missing main image");
assert.match(appSource, /aria-label=\{isThumbnailToSnsHandoffPayload\(handoffPayload\) \? copy\.handoff\.thumbnailReceived : copy\.handoff\.scheduleReceived\}/, "handoff status label names the source");
...
assert.match(copySource, /横長2枚を保存順どおりに作ります。/, "split-2 landing copy is short and save-order oriented");
