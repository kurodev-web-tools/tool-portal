...
try{
 for(const image of ['public.ecr.aws/supabase/postgres:17.6.1.140','public.ecr.aws/supabase/gotrue:v2.192.0','public.ecr.aws/supabase/mailpit:v1.30.2','public.ecr.aws/supabase/postgrest:v14.14','public.ecr.aws/supabase/realtime:v2.112.1',nodeImage])docker(['image','inspect',image]);
 ...
 // Prepare the independent destination before minting short-lived source material.
 const sourceContext={db,client,auth};
 phase='destination_setup';db=run('destination-db','public.ecr.aws/supabase/postgres:17.6.1.140',{POSTGRES_PASSWORD:password});
 ready=false;for(let i=0;i<60;i++){if(docker(['exec',db,'pg_isready','-h','127.0.0.1','-U','postgres'],null,true).status===0){ready=true;break;}await wait(500);}assert.ok(ready);
 sql('ALTER ROLE supabase_auth_admin PASSWORD '+lit(password)+'; ALTER ROLE authenticator PASSWORD '+lit(password)+';','supabase_admin');
 client=run('destination-client',nodeImage,{},['--entrypoint','node'],['-e','setInterval(()=>{},100000);']);
 run('destination-smtp','public.ecr.aws/supabase/mailpit:v1.30.2');
 auth=await startAuth('destination-auth-closed',newKey,{GOTRUE_DISABLE_SIGNUP:'true',GOTRUE_EXTERNAL_EMAIL_ENABLED:'false'});
 ...
 sql("CREATE TABLE public.ct_gate1_recovery_probe(id integer PRIMARY KEY); ALTER TABLE public.ct_gate1_recovery_probe ENABLE ROW LEVEL SECURITY; REVOKE ALL ON public.ct_gate1_recovery_probe FROM PUBLIC,anon,authenticated; INSERT INTO public.ct_gate1_recovery_probe VALUES(17);");
 sql("CREATE FUNCTION public.ct_gate1_rehearsal_rpc() RETURNS integer LANGUAGE sql STABLE SET search_path=pg_catalog AS 'SELECT 17';");
 phase='data_api_configuration';
 const restBefore={db_schema:'public, graphql_public',max_rows:1000,db_extra_search_path:'public, extensions',db_pool:10,db_pool_acquisition_timeout:10};
 const restChange=buildRehearsalServiceChange('postgrest',restBefore,{db_schema:''});
 const rest=run('rest','public.ecr.aws/supabase/postgrest:v14.14',{PGRST_DB_URI:`postgres://authenticator:${password}@127.0.0.1:5432/postgres?sslmode=disable`,PGRST_DB_SCHEMAS:restBefore.db_schema,PGRST_DB_ANON_ROLE:'anon',PGRST_DB_CONFIG:'true',PGRST_DB_MAX_ROWS:'1000',PGRST_DB_POOL:'10',PGRST_DB_POOL_ACQUISITION_TIMEOUT:'10',PGRST_DB_EXTRA_SEARCH_PATH:restBefore.db_extra_search_path,PGRST_JWT_SECRET:newKey,PGRST_SERVER_HOST:'127.0.0.1',PGRST_SERVER_PORT:'3001'});
 ready=false;for(let i=0;i<30;i++){const response=businessApi('/user_profiles?select=user_id');if(response.status===401&&response.data?.code==='42501'){ready=true;break;}await wait(500);}assert.ok(ready);
 const rpcControl=businessApi('/rpc/ct_gate1_rehearsal_rpc',adminToken(newKey),{});assert.equal(rpcControl.status,200);assert.equal(rpcControl.data,17);
 const graphqlControl=businessApi('/rpc/graphql',adminToken(newKey),{query:'{__typename}'},{'Content-Profile':'graphql_public'});
 report.graphqlControl={status:graphqlControl.status,code:graphqlControl.data?.code??null,queryType:graphqlControl.data?.data?.__typename??null,errorCount:graphqlControl.data?.errors?.length??0};assert.equal(graphqlControl.status,200);assert.deepEqual(graphqlControl.data,{data:{__typename:'Query'}});
 restState={db,container:rest};
 // PostgREST 14.14 treats an empty setting as its public default. An explicitly
 // absent schema closes this local service. This is not the Hosted mapping of
 // Management API db_schema=''; Hosted must independently return 503/PGRST002.
 sql("ALTER ROLE authenticator SET pgrst.db_schemas='ct_rehearsal_closed'; NOTIFY pgrst,'reload config'; NOTIFY pgrst,'reload schema';");
 ready=false;for(let i=0;i<30;i++){try{requireBusinessClosed();ready=true;break;}catch{}await wait(500);}assert.ok(ready);
 report.localDataApiClosure='absent-schema-sentinel';report.managementEmptySchemaMappingProven=false;
 phase='storage_realtime_setup';
 const services=await startRehearsalLocalServices({run,docker,inspect,sql,client,nodeImage,password,signingKey:newKey,serviceToken:adminToken(newKey),anonToken:adminToken(newKey,'anon'),onPhase:value=>{phase=value;},onObservation:value=>{report.serviceFailure=value;}});
 report.localServices=services.result;
 const destinationContext={db,client,auth};assert.notEqual(db,sourceContext.db);
 ({db,client,auth}=sourceContext);
 ...
 phase='synthetic_transfer';docker(['stop','--time','1',auth]);
 ...
 const sourceShape=sql('SELECT '+ATOMIC_MANAGED_SHAPE_SQL+';');
 const sourceTables=JSON.parse(sql('SELECT jsonb_build_array('+REHEARSAL_TRANSFER_TABLES.map(name=>"jsonb_build_object('name',"+lit(name)+",'columns',("+rehearsalColumnsSql(name)+"),'rows',("+rehearsalRowsSql(name)+"))").join(',')+');'));
 ({db,client,auth}=destinationContext);
 ...
 const beforeTransfer=sql(REHEARSAL_READBACK_SQL);
 validateRehearsalPreparation(fixture);
 const packet={scope:'LOCAL_SYNTHETIC_ONLY',managedShapeSha256:sourceShape,targetBaselineSha256:hash(beforeTransfer),userIds:users.map(u=>u.id),tables:sourceTables};
 const target=inspect(db),targetId=db;
 const verifyTarget=()=>{const v=inspect(targetId);assert.equal(db,targetId);assert.equal(v.Id,target.Id);assert.equal(v.Image,target.Image);assert.equal(v.Config.Labels['com.comment_translator.atomic'],owner);assert.equal(v.HostConfig.NetworkMode,'none');assert.equal(Object.keys(v.HostConfig.PortBindings??{}).length,0);assert.equal(v.State.Running,true);return true;};
 let observedBaseline=false;
 const executeTransferSql=async q=>{const raw=sql(q);if(q===REHEARSAL_READBACK_SQL&&!observedBaseline){
  observedBaseline=true;const before=JSON.parse(beforeTransfer),after=JSON.parse(raw);
  const identities=JSON.parse(sql("SELECT jsonb_object_agg(encode(sha256(convert_to(jsonb_build_array(n.nspname,c.relname)::text,'UTF8')),'hex'),jsonb_build_array(n.nspname,c.relname)) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname !~ '^pg_' AND n.nspname<>'information_schema' AND c.relkind IN ('r','p','S');"));
  report.transferBaseline={bytesMatched:hash(raw)===hash(beforeTransfer),securityMatched:before.security===after.security,definitionsMatched:before.definitions===after.definitions,changedRelations:[...new Set([...Object.keys(before.tables),...Object.keys(after.tables)])].filter(k=>before.tables[k]!==after.tables[k]).map(k=>identities[k]??['unknown','unknown'])};
 }return raw;};
 report.transfer=await createRehearsalExecutor({requiresTls:false,verify:verifyTarget,claim:()=>nativeRehearsalAttemptLedger().claim(hash('local-container:'+targetId)),execute:executeTransferSql,stop:async()=>{verifyTarget();docker(['stop','--time','1',targetId]);return inspect(targetId).State.Running===false;}}).run(packet,{assertFreshFixtures:()=>{validateRehearsalPreparation(fixture);return true;}});
 assert.equal(sql('SELECT count(*) FROM auth.flow_state;'),'1');
 assert.equal(sql('SELECT id FROM public.ct_gate1_recovery_probe;'),'17');
 report.exactResetAndReadback=true;report.independentSourceDestination=true;report.unrelatedDestinationRowsPreserved=true;
 ...
 const before=fullState(),mailBefore=closedMailCount();assert.equal(mailBefore,0);
 ...
 checkOldLinks();
 phase='closed_service_matrix';
 const probeServices=tokens=>{
  const baseline=sql(REHEARSAL_READBACK_SQL),observations=[];requireBusinessClosed();
  for(const {kind,token}of tokens){for(const [route,path,body,headers,method]of [
   ['rest-read','/user_profiles?select=user_id',null,{},'GET'],
   ['rest-write','/user_profiles',{user_id:users[0].id,display_name:'Blocked'}, {},'POST'],
   ['rpc','/rpc/ct_gate1_rehearsal_rpc',{}, {},'POST'],
   ['graphql','/rpc/graphql',{query:'{__typename}'},{'Content-Profile':'graphql_public'},'POST'],
   ['graphql-direct','/rpc/graphql',{query:'{__typename}'},{},'POST'],
  ]){const r=businessApi(path,token,body,headers,method),observation={kind,route,status:r.status,code:r.data?.code??null};observations.push(observation);report.lastServiceProbe=observation;
   // Invalid source JWTs are rejected before schema-cache handling. Closing
   // business exposure is separately proved with valid target anon/user JWTs.
   assert.equal(r.status,kind==='old-access'?401:503);assert.equal(r.data?.code,kind==='old-access'?'PGRST301':'PGRST002');}}
  const storage=services.probeStorage(tokens),realtime=services.probeRealtime();
  const after=sql(REHEARSAL_READBACK_SQL);report.serviceMatrixFingerprintMatched=after===baseline;assert.equal(after,baseline);
  return {dataApi:observations,storage,realtime,allDatabaseStateUnchanged:true,inventory:services.inventory()};
 };
 report.closedServiceMatrix=probeServices([{kind:'anon',token:adminToken(newKey,'anon')},{kind:'old-access',token:users[0].accessToken}]);
 ...
 report.authReopenedServiceMatrix=probeServices([{kind:'anon',token:adminToken(newKey,'anon')},{kind:'authenticated',token:reopened[0].access_token}]);
 report.oldCredentialsAfterReopenNoAuthDelta=true;
 report.p2P6AuthAndDirectBusinessLocalPass=true;
 phase='business_api_reopen';
 // Restore the captured schema exposure only after Auth/direct DB acceptance.
 const reopenRest=buildRehearsalServiceChange('postgrest',restChange.after,restChange.inverse);
 sql('ALTER ROLE authenticator SET pgrst.db_schemas='+lit(reopenRest.patch.db_schema)+"; NOTIFY pgrst,'reload config'; NOTIFY pgrst,'reload schema';");
 compareConfigurationReadback(reopenRest,{...restBefore,db_schema:sql("SELECT split_part(v,'=',2) FROM pg_roles r,unnest(r.rolconfig) v WHERE r.rolname='authenticator' AND v LIKE 'pgrst.db_schemas=%';")});
 ...
 report.scope='LOCAL_SYNTHETIC_TRANSFER_AND_REOPEN';report.status='PASS';
}catch(e){report.phase=phase;report.sqlState=e.sqlState??null;report.adapterPhase=e.phase??null;report.reason=e.reason??null;report.failureClass=e.code??(['TypeError','ReferenceError'].includes(e.name)?e.name:'SANITIZED_FAILURE');process.exitCode=1;}
finally{
 ...
 fs.writeFileSync('.tmp/gate1-evidence-20260910/rehearsal-transfer-local-'+owner+'.json',JSON.stringify({...report,stageAuthority:false,gate:'NO-GO'}),{flag:'wx'});
