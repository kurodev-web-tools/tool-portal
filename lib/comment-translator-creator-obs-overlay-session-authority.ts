...
export const commentTranslatorCreatorObsOverlaySessionAuthorityContract = {
  implementationStage: "nc-o2-local-obs-overlay-session-authority",
  runtime: "server-only",
...
export function isCommentTranslatorCreatorObsOverlayBrowserRouteClosed({
...
export function createCommentTranslatorCreatorObsOverlaySessionAuthority({
  ...
  return {
    async readCurrentForOwner(ownerUserId, nowMs = Date.now()): Promise<CommentTranslatorCreatorObsSessionAuthorityResult> {
      if (durableSessionStore.status !== "ready") {
        return { status: "unavailable", reason: "session-authority-unavailable" };
      }
      if (!Number.isFinite(nowMs)) return { status: "unavailable", reason: "session-authority-unavailable" };
      try {
