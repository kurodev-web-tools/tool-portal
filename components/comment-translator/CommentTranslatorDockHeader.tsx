  ...
  return (
    <section className="panel p-4 shadow-sm sm:p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <div className="rounded-base border border-border bg-surface px-4 py-3"><p className="text-xs font-black text-primary-strong">{locale === "ja" ? "プラン" : "Plan"}</p><div className="mt-2 flex flex-wrap items-center justify-between gap-2"><p className="break-words text-base font-black text-foreground">{plan === "paid" ? "Pro" : "Free"}</p><span className="rounded-base border border-primary/30 bg-primary-soft px-2.5 py-1 text-xs font-black text-primary-strong">{locale === "ja" ? "利用可" : "Available"}</span></div></div>
      </div>
    </section>
  );
}
