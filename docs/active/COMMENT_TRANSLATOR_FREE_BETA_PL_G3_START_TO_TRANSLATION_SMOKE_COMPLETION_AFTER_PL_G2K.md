...
Keep public launch blocked. The approved same-process target-refresh diagnostic confirms the non-empty intake path when target refresh and bounded polling share the selected server-only target in one process. The next safe step is a separate exact same-thread approval for the PL-G3 post-bridge full Start-to-translation continuation that includes status precheck, explicit Start, server-only target lookup or same-process target preservation, bounded polling, Free provider translation, browser-visible server-owned feed confirmation, source attribution, usage/session counter evidence, Stop, and post-Stop status. Free provider execution, browser-visible feed confirmation, PL-G4, PL-G5, deploy/upload, remote mutation outside the approved reset boundary, OAuth flows, token refresh, Stripe actions, public access changes, main promotion, and public launch gate flip remain blocked until separately approved.
