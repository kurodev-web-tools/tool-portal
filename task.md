...
- This branch follows up merged PR #529 after the reviewed root-cause triage recorded stale/wrong live target reference or operator-visible chat surface mismatch as the smallest remaining PL-G3 gap. This implementation prepares the same-process target-refresh-to-bounded-polling diagnostic command boundary only; it does not run live/provider execution. The future command blocks before provider access unless the exact value-free approval label is present, refreshes owned live target lookup in the same command process, keeps the selected target value in memory only, then passes that selected target to bounded short polling. Start, Stop, target lookup execution, `liveChatMessages.list`, Azure/OpenAI provider execution, UI/feed confirmation, production/custom deployed smoke execution, deploy/upload, remote Supabase mutation/schema apply, limited public beta open, public access change, public launch gate flip, main promotion, heartbeat mutation, additional provider target lookup, additional live target lookup, Stripe action, billing setting mutation, Paid entitlement C1/C3, Creator paid limits, cursor regeneration, OAuth flows, and token refresh are not run. PL-G3 remains blocked-same-process-target-refresh-to-bounded-polling-command-prepared-after-pr529 / Azure-UI-not-run / public-release capable no. PL-G4 remains production/custom deployed smoke not-run / approval-gated, PL-G5 remains keep blocked / blocked-no-approval, public gate state label unchanged / blocked, and public-release capable label no.
