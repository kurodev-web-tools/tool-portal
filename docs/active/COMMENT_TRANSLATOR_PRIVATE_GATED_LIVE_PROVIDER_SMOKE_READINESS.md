...
Status: Task 27 readiness/blocker evidence before private-gated live/provider smoke. Public-release capable: no.
...
Task 27 completion criteria are not met in this readiness/blocker PR because approved live/provider smoke was not run and the existing polling smoke command cannot yet prove the full live intake plus translation path.
...
4. `translatorPipelineWiring: not-implemented` on the current Live Chat polling smoke command, so it can prove a bounded polling smoke gate but not the full live comment intake plus translation path.
5. No approved bounded execution evidence for stop behavior, quota/budget stop behavior, or provider translation execution.

## Next Safe Action

After this readiness/blocker PR is merged, continue Task 27 in a fresh branch from `origin/codex/comment-translator-preview`.
...
5. Run the narrow approved command only, with sanitized evidence recording.
