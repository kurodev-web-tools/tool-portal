...
1. Kuro Live Comment Translator public release roadmap
   ...
   - current PR scope: Task 5 YouTube OAuth account integrations status wiring from trusted credential status, no live connect execution. This PR targets `codex/comment-translator-youtube-oauth-integration`; it reads the deterministic owner-bound credential reference through a server-only trusted status boundary, maps connected / reconnect-required / disconnected / unavailable / error to sanitized `/account/integrations` copy, and wires safe disconnect to existing trusted credential invalidation without provider revoke/live API execution.
   ...
   - current feature branch: `codex/comment-translator-youtube-oauth-account-status-wiring`, targeting `codex/comment-translator-youtube-oauth-integration`.
   ...
   - current OAuth blockers: `app/account/actions.ts` connect/reconnect construct Google OAuth authorization redirects through a server-only boundary when env references and private-launch/auth gates are ready. Callback handling delegates valid callbacks to the Task 4 server-only persistence wiring. `/account/integrations` now reads trusted credential status for sanitized account status, but `/tools/comment-translator` still uses the fixed preview credential reference and trusted-adapter-not-wired metadata until Task 6.
   ...
   - verification for this branch: `node scripts/comment-translator-youtube-oauth-account-status-wiring-contract.mjs`, changed-files no-secret scan, `npm run lint`, `npx tsc --noEmit`, `npm run build`, and `git diff --check` passed locally. Existing Task 4/3/2 focused contracts were inspected and attempted, but their task-specific changed-file allowlists are no longer applicable to this Task 5 UI/action/status slice and fail on expected Task 5 files rather than the Task 4/3/2 boundaries.
   ...
   - next safe action: after Task 5 PR merges into `codex/comment-translator-youtube-oauth-integration`, start Task 6 tool credential source wiring away from fixed preview credential reference in a fresh worktree / feature branch. Do not run Google OAuth live connect, YouTube OAuth live connect, live authorization code exchange, live token persistence, provider target lookup, liveChatId lookup, session start smoke, deploy/upload, remote mutation, remote schema migration, Stripe live-mode action, Customer Portal redirect, webhook registration, translation provider API execution, or live/provider execution without same-thread ready preflight, sanitized output review, and explicit approval.
