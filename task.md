...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-credential-status-source-evidence-post-pr306-review` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-credential-status-source-evidence-post-pr306-review`
   - seed:
   ...
   - current slice target:
     - `post-PR #306 source/evidence review gate` を PR #306 (`[codex] Gate post-PR305 credential source evidence review`) merge 後の follow-up として確認する。
     - 前提 PR #306 は 2026-06-03 に `codex/comment-translator-preview` へ merge 済みで、merge commit `650917f72963bf82649ec39142e3c54f2db6bc4e` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     ...
     - `scripts/comment-translator-youtube-credential-reference-surface-source-recheck-contract.mjs` と `lib/comment-translator-youtube-client-safe-credential-reference-source.ts` の post-PR #306 source/evidence review gate helper で、PR #306 merge 後も source と evidence が両方揃わない状態を `blocked-pr306-source-evidence-review-missing-surfaced-source-or-approval-evidence` として扱う。
