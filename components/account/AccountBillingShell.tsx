...
const billingCopy = {
  ja: {
    ...
    providerRetention: "OpenAIでは標準のabuse monitoringにより最大30日保持される可能性があります。Azure TranslatorはMicrosoftのNo-Trace方針を前提とします。Providerごとの条件は公開前に再確認します。",
    accountDispute: "アカウント削除は原則として期間終了時解約後に行います。dispute利用者勝訴時は対象ownerのPaid即時停止、idempotentなcancel、canceled確認後のcapacity解放とし、失敗時は停止・保持・manual reconciliationです。dispute運営勝訴時は現在Subscription/periodが有効で、他の停止理由がない場合だけPaidを復元し、不明なら停止・保持・manual reconciliationです。",
  ...
  en: {
    ...
    providerRetention: "OpenAI may retain data for up to 30 days through standard abuse monitoring. Azure Translator is described under Microsoft's No-Trace policy. Provider-specific terms are rechecked before launch.",
    accountDispute: "Account deletion generally cancels the contract at period end, followed by deletion of app-side account data. For a dispute won by the user, stop Paid for the targeted owner, request an idempotent cancel, and release capacity only after canceled is confirmed; failures keep the stop and hold state for manual reconciliation. For a dispute won by the operator, restore Paid only when the current Subscription/period is valid and no other stop reason exists; otherwise keep the stop and hold state for manual reconciliation.",
...
export function AccountBillingShell({ accountStatus, billingMessage, billing, createCheckoutAction, createPortalAction }: { accountStatus: AccountSessionBrowserSafeViewModel; billingMessage: string | null; billing: CommentTranslatorBillingBrowserSafeViewModel; createCheckoutAction: AccountFormAction; createPortalAction: AccountFormAction }) {
  ...
  return <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
    ...
    <section data-comment-translator-plan-comparison="free-and-paid-server-derived" className="grid gap-4">
      ...
      <div id="comment-translator-paid-conditions" data-comment-translator-paid-safety="billing-period-maximum-not-guarantee" className="rounded-base border border-primary/30 bg-primary-soft/45 px-4 py-3"><h2 className="text-sm font-black text-primary-strong">{copy.paidPeriod}</h2><p className="mt-2 break-words text-sm font-bold leading-7 text-foreground">{copy.paidPeriodBody}</p><p className="mt-2 break-words text-sm font-semibold leading-6 text-muted">{copy.salesAndPayment}</p><p className="mt-2 break-words text-sm font-semibold leading-6 text-muted">{copy.privacyDataBoundary}</p><p className="mt-2 break-words text-sm font-semibold leading-6 text-muted">{copy.providerRetention}</p><p className="mt-2 break-words text-sm font-semibold leading-6 text-muted">{copy.accountDispute}</p></div>
    </section>
