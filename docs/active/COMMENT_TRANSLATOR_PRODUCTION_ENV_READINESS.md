...
| `COMMENT_TRANSLATOR_CREATOR_CLOSED_BETA_BILLING_ACCESS` | activation-only | server var | Exact C2 closed-beta Checkout / Portal / signed-webhook activation marker | Missing or non-exact values keep Checkout/Portal unavailable and reject webhook entitlement sync as `missing-config`. Defining this reference is not approval to set it. | Cloudflare production server var only during a separately approved activation; value not recorded. |
...
| `COMMENT_TRANSLATOR_PAID_TRANSLATION_PROVIDER` | smoke-only | server var | C4 Paid provider route selection | Missing or values other than the exact reviewed OpenAI-mini marker fail closed before paid provider invocation. | Cloudflare production var and operator-local smoke env before approved paid provider execution. |
| `COMMENT_TRANSLATOR_TRANSLATION_MONTHLY_BUDGET_USD` | smoke-only | server var | C4 paid provider budget ceiling | Missing, non-numeric, non-positive, or unsafe values fail closed before paid provider invocation. | Cloudflare production var and operator-local smoke env before approved paid provider execution. |
| `COMMENT_TRANSLATOR_TRANSLATION_BUDGET_SOFT_STOP_RATIO` | smoke-only | server var | C4 provider budget policy configuration | Missing or invalid values make the approved budget configuration incomplete and fail closed. C4 does not infer a replacement value. | Cloudflare production var and operator-local smoke env before approved paid provider execution. |
| `COMMENT_TRANSLATOR_TRANSLATION_BUDGET_HARD_STOP_RATIO` | smoke-only | server var | C4 provider budget hard stop | Missing, invalid, or non-increasing soft/hard ratios fail closed; C3 current-period estimated cost at the hard stop blocks provider invocation. | Cloudflare production var and operator-local smoke env before approved paid provider execution. |
| `COMMENT_TRANSLATOR_AZURE_MONTHLY_CHARACTER_CAP` | smoke-only | server var | C4 Azure fallback budget cap | Missing or exhausted cap disables Azure fallback without changing the OpenAI primary decision. | Cloudflare production var and operator-local smoke env before approved Azure fallback execution. |
...
| `AZURE_TRANSLATOR_ENDPOINT` | optional | server var | Azure Translator endpoint override and C4 fallback readiness | Free may use the runtime default; C4 Paid fallback remains disabled unless the approved endpoint reference is explicit. | Cloudflare production var before approved C4 Azure fallback execution. |
| `AZURE_TRANSLATOR_REGION` | optional | server var | Azure Translator regional header and C4 fallback readiness | Free behavior depends on the Azure resource; C4 Paid fallback remains disabled unless the approved region reference is explicit. | Cloudflare production var before approved C4 Azure fallback execution. |
