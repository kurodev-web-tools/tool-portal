...
- Current branch: `codex/comment-translator-admin-dashboard-shortcut`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- Scope: Comment Translator admin shortcut + dashboard shell. Allowlisted Comment Translator admins get a server-allowlist-resolved shortcut in the portal navigation, `/admin/comment-translator` provides a small admin dashboard shell, and the shell links to `/admin/comment-translator/creator-waitlist`. Rate-limit tools: planned-only. No rate-limit unblock/reset action, admin mutation endpoint/server action, live/provider execution, deploy/upload, remote migration apply, Stripe, OAuth live flow, Google target lookup, OBS, moderation controls, or public gate flip is in scope.
...
3. `Comment Translator admin shortcut + dashboard shell`: current branch; exposes the admin area from existing portal navigation only after the server-side admin hash allowlist check, adds `/admin/comment-translator`, links to the Creator waitlist admin page, and keeps rate-limit operations planned-only/non-mutating.
