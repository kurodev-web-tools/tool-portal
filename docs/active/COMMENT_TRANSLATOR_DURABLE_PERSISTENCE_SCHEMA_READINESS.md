...
| entitlement persistence | Local F5 public entitlement baseline resolver exists for Free; Paid durable entitlement remains future C1 | Free public beta uses safe Free degradation and adds 20,000 provider-input characters/month on top of existing time/per-minute/active-session caps; Durable Paid rows are required before Paid limits can be relied on publicly | Yes | Sanitized plan/limit metadata only |
...
| F14 Creator locked cards / waitlist / click tracking | Local deterministic Creator locked/waitlist resolver records price intent, locked feature cards, waitlist availability, and click-tracking readiness from server-owned durable/readiness inputs | Paid access is not live; click tracking is local deterministic draft only, remote persistence and Stripe live actions remain approval-gated/not-run | Yes | Sanitized Creator intent, locked feature labels, waitlist state, and click draft status only |
| F15 Free public beta final QA / launch readiness | Free beta-specific readiness record exists with route/API smoke plan, no-secret scan plan, width-check decision, legal/copy review, rollback notes, blockers, accepted risks, unchecked scope, and launch decision | Public-release capable: no; live/provider/remote/Stripe evidence and launch gate flip remain approval-gated/not-run | Yes | Sanitized readiness labels and not-run gates only |
| FB-L2 Remote durable enforcement evidence | Blocker/evidence record exists for remote/deployed durable session and usage enforcement. `comment_translator_sessions` and `comment_translator_usage_ledger_events` remain the required Free durable authority, but same-thread approval was not present. | Remote Supabase migration apply, remote mutation, and deployed durable write/read smoke remain approval-gated/not-run; public-release capable remains no | Yes | Sanitized status labels, table names, counts/status/stop reasons only |
...
- F12 Usage display for Free beta. This PR adds browser-safe Free beta usage display metadata for session, day, per-minute, and monthly provider-input character caps. The display is derived from server-owned usage and entitlement state, keeps provider target metadata, raw provider payloads, raw comments, tokens, account ids, live target values, and server-only cursors out of browser-readable output, and blocks translation provider calls before execution when usage is over limit or unavailable.
...
- F14 Creator locked cards / waitlist / click tracking. This PR adds a server-only local resolver and UI/action path for Creator price intent, locked feature cards, waitlist state, and sanitized click draft status during Free public beta. Paid access is not live, checkout is not available, and remote waitlist/click persistence, Stripe live actions, provider execution, deploy/upload, remote schema/migration apply, and public launch remain not-run/approval-gated.
- F15 Free public beta final QA / launch readiness. This PR records the final Free beta readiness decision, route/API smoke plan split into local deterministic, sanitized server-owned state, and approval-gated exact-command preflight lanes, changed-files no-secret scan plan, width-check skip reason, legal/copy review, rollback notes, blockers, accepted risks, unchecked scope, and `public-release capable: no`. It does not run live/provider execution, deploy/upload, remote schema/migration apply, Stripe live actions, main promotion, or public launch gate flip.
- FB-L1 remote/deployed durable enforcement preflight. `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PUBLIC_USABILITY_PREFLIGHT.md` defines the exact order and evidence boundary for proving deployed Free durable session/usage enforcement later. The proof still requires a same-thread approved remote/deployed smoke; FB-L1 does not run remote Supabase migration apply, remote mutation, deploy/upload, session start, provider lookup, polling, Azure/OpenAI provider execution, Stripe live action, or public launch gate flip.
- FB-L2 Remote durable enforcement evidence. `docs/active/COMMENT_TRANSLATOR_FREE_BETA_REMOTE_DURABLE_ENFORCEMENT_EVIDENCE.md` records blocked-no-approval because same-thread ready preflight, sanitized output review, and exact explicit approval were not present. It clarifies what remote/deployed durable enforcement can prove later and what it does not prove, while keeping remote Supabase migration apply and mutation not-run / approval-gated.
...
- F5 public entitlement baseline is implemented locally, but public launch remains blocked. The resolver uses durable F4 monthly usage reads for the 20,000 provider-input characters/month cap and degrades any non-durable Paid entitlement to Free until durable Paid entitlement work lands.
...
- F14 Creator locked cards / waitlist / click tracking is implemented locally, but public launch remains blocked. Creator price intent, locked cards, waitlist state, and click tracking are deterministic/local and sanitized; remote waitlist/click persistence, Stripe live actions, paid entitlement activation, real provider target lookup, live target lookup, `liveChatMessages.list`, Azure/OpenAI provider API execution, remote mutation/schema apply, deploy/upload, and launch gate flip are not run in this thread.
- F15 Free public beta final QA / launch readiness is implemented as docs/contract readiness, but public launch remains blocked. The readiness decision is `public-release capable: no`; real provider target lookup, live target lookup, `liveChatMessages.list`, Azure/OpenAI provider API execution, session start smoke, remote mutation/schema apply, Stripe live action, deploy/upload, main promotion, and public launch gate flip are not run in this thread.
