...
1. Kuro Live Comment Translator preview branch
   ...
   - immediate next condition: record and verify the target lookup execution evidence PR, then after merge proceed to Live Chat polling smoke readiness using operator-local target metadata only. Do not put `liveChatId` value in docs / PR body / output.
   - next PR candidate: Live Chat target lookup execution evidence PR. Then Live Chat polling smoke execution evidence PR after target lookup presence-only evidence is confirmed in the polling command's operator-local context.
