...
assert.match(beginCheckoutSource, /v_hold_id := gen_random_uuid\(\)[\s\S]+?v_idempotency_key := 'ct-paid-checkout-' \|\| v_hold_id::text[\s\S]+?v_checkout_expires_at_target := date_trunc\('second', p_now\) \+ interval '31 minutes'/i, "Checkout begin derives canonical key and UTC-second target from hold ID and statement time");
...
assert.doesNotMatch(azureSource, /(?:insert\s+into|update|delete\s+from)\s+public\.comment_translator_paid_(?:openai_slots|openai_minute_buckets|openai_rate_reservations)/i, "Azure direct fallback does not mutate OpenAI slots or rate; it only gates on the existing cost authority");
assert.match(azureSource, /v_owner_cost[\s\S]+?v_global_cost[\s\S]+?individual Paid cost limit is exhausted[\s\S]+?global Paid cost limit is exhausted/i, "Azure direct fallback gates on readable individual and global cost authorities without reserving OpenAI cost");
