...
for (const requiredPath of [
  ...
  assert.ok(exists(requiredPath), `PL-G2F required path exists: ${requiredPath}`);
}
...
for (const requiredSection of [
  ...
  assert.match(plG2fDoc, new RegExp(`^${escaped(requiredSection)}$`, "m"), `PL-G2F doc includes ${requiredSection}`);
}

for (const requiredFragment of [
  "Status: PL-G2F allowed-tester route/API harness smoke execution gate after PL-G2E execution gate",
  ...
  assert.match(plG2fDoc, new RegExp(escaped(requiredFragment), "i"), `PL-G2F doc includes ${requiredFragment}`);
}
...
for (const forbiddenFragment of [
  ...
  assert.doesNotMatch(plG2fDoc, new RegExp(escaped(forbiddenFragment), "i"), `PL-G2F doc excludes ${forbiddenFragment}`);
}

assert.match(plG2eDoc, /keep blocked \/ blocked-no-approval/i, "PL-G2E follow-up stays blocked");
...
assert.match(allowedTesterEvidence, /PL-G2F[\s\S]*keep blocked \/ blocked-no-approval/i, "FB-L3 evidence records PL-G2F follow-up");
...
assert.match(publicUsabilityPreflight, /PL-G2F[\s\S]*allowed-tester route\/API harness smoke/i, "FB-L1 public usability preflight records PL-G2F");
assert.match(finalQa, /PL-G2F[\s\S]*keep blocked \/ blocked-no-approval/i, "F15 readiness records PL-G2F follow-up");
assert.match(gapAudit, /PL-G2F[\s\S]*allowed-tester route\/API harness smoke/i, "gap audit records PL-G2F");

assert.match(routeHarness, /COMMENT_TRANSLATOR_FREE_BETA_ROUTE_API_HARNESS_ENABLED/, "PL-G2F route harness remains env-gated");
assert.match(routeHarness, /x-comment-translator-harness-approval/, "PL-G2F route harness remains approval-header gated");
assert.match(routeHarness, /readCommentTranslatorPrivateLaunchAccess[\s\S]*status:\s*403/, "PL-G2F route harness remains private-launch gated");
assert.match(routeHarness, /getCommentTranslatorRealCommentsFeedAction/, "PL-G2F route harness includes feed action");
assert.match(routeHarness, /requestCommentTranslatorDataDeletionAction/, "PL-G2F route harness includes deletion action");
assert.match(routeHarness, /getCommentTranslatorCreatorLockedWaitlistAction/, "PL-G2F route harness includes Creator locked waitlist action");
assert.match(routeHarness, /recordCommentTranslatorCreatorLockedClickAction/, "PL-G2F route harness includes Creator locked click action");
assert.doesNotMatch(routeHarness, /return NextResponse\.json\(\s*(feed|state|draft|deletion)/, "PL-G2F harness does not return raw action payloads");
assert.match(sessionRoute, /readSessionCommand[\s\S]*normalizeCommandBody[\s\S]*intent[\s\S]*status/, "PL-G2F status route accepts status intent");
assert.match(sessionRoute, /readCommentTranslatorPrivateLaunchAccess[\s\S]*status:\s*403/, "PL-G2F status route remains private-launch gated");
assert.match(actions, /getCommentTranslatorRealCommentsFeedAction[\s\S]*live-provider-polling-not-approved/, "PL-G2F feed action remains provider-unavailable");

assert.match(
  task,
  /Current branch: `codex\/comment-translator-free-beta-pl-g2(?:f-route-api-harness-smoke-execution-gate-after-pl-g2e|g-route-api-harness-smoke-execution-after-pl-g2f|h-approved-route-api-harness-smoke-execution-after-pl-g2g|i-approved-route-api-harness-smoke-execution-after-pl-g2h|j-approved-route-api-harness-smoke-execution-after-pl-g2i|k-approved-route-api-harness-smoke-execution-after-pl-g2j)`/i,
  "task.md records PL-G2F, PL-G2G, PL-G2H, PL-G2I, PL-G2J, or PL-G2K branch"
);
assert.match(task, /Latest PL-G2F Execution Gate Evidence/i, "task.md records Latest PL-G2F Execution Gate Evidence");
assert.match(task, /PL-G2F execution gate[\s\S]*keep blocked \/ blocked-no-approval/i, "task.md records PL-G2F blocked result");
assert.match(task, /approved-fb-l3-allowed-tester-route-api-smoke/i, "task.md records PL-G2F approval label");
...
assert.match(task, /residual risk[\s\S]*PL-G2 remains incomplete/i, "task.md records PL-G2F residual risk");
assert.match(task, /Next safe action[\s\S]*keep PL-G2 blocked[\s\S]*approved-fb-l3-allowed-tester-route-api-smoke/i, "task.md records PL-G2F next safe action");
assert.match(task, /width checks skipped[\s\S]*no visible UI\/CSS\/layout\/copy change/i, "task.md records PL-G2F width-check skip reason");
...
for (const file of changedFiles()) {
  assert.ok(allowedChangedFiles.has(file), `PL-G2F execution gate change stays in allowed files: ${file}`);
  assertNoSensitiveValues(read(file), `changed file ${file}`);
}

console.log("comment translator Free beta PL-G2F route/API harness smoke execution gate contract checks passed");
