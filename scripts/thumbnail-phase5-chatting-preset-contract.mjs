...
const chattingPreset = lib.thumbnailPresets.find((item) => item.id === "chatting");
assert.ok(chattingPreset, "chatting preset exists");

const background = chattingPreset.layers.find((layer) => layer.type === "image" && layer.name.includes("背景"));
assert.ok(background, "chatting has a background image layer");
assert.equal(background.src, expectedBackground, "chatting uses the phase 5 generated background");
assert.equal(background.locked, true, "chatting phase 5 background is locked by default");
const backgroundPath = path.join(root, "public", expectedBackground.replace(/^\//, ""));
assert.equal(fs.existsSync(backgroundPath), true, "chatting phase 5 generated background exists");
...
for (const fileName of expectedDecorationFiles) {
  assert.equal(decorationSources.has(fileName), true, `chatting preset uses ${fileName}`);
}

for (const roleName of ["見出し", "時刻", "サブ", "ラベル"]) {
  assert.ok(
    chattingPreset.layers.some((layer) => layer.type === "text" && layer.name.includes(roleName)),
    `chatting keeps editable ${roleName} text layer`
  );
}
...
assert.equal(
  ...
  expectedBackground,
  "phase 5 chatting background survives draft normalization"
);

console.log("thumbnail phase 5 chatting preset contract checks passed");
