...
- `CP1-A-STRIPE-CHECKOUT-RUNTIME-READINESS-PREFLIGHT-1` is consumed/fail-closed from exact integration `f5e81b21382b1d07b9d5dd20f78c2fbbceb9e5c5`: production presence evidence is `route unavailable / 5 unreviewed`, and C1 durable billing-state production read remains `disconnected-fail-closed`. Exactly one blocker, `c1-durable-billing-state-read-disconnected`, stops the sequence before Stripe SDK initialization or Checkout creation. Free / paid-inactive remain unchanged and `CP1-A-STRIPE-CHECKOUT` remains non-executable.
- `CP1-A-C1-PRODUCTION-DURABLE-READ-WIRING-1` is reviewable from exact base `3a3dd3631d37ea5f5ab8bfffdc1318eb25f828a3`. The C1 read is `connected-unactivated`; the next blocker is `c1-production-activation-read-proof-pending`. Deploy, production read, configuration apply, activation, and Checkout remain not-run, and the canonical detail is the external-evidence reconciliation authority.
- `CP1-A-C1-PRODUCTION-ACTIVATION-READ-PROOF-1` is locally reviewable from exact base `b0fe19823e260d768749604affa57cf30d3c7329`. Its one sanitized Cloudflare metadata inspection stopped at `cloudflare-deployment-status-inspection-unavailable`, so exact deployed revision and remote binding/Container presence remain unproven. Activation, production C1 read, Checkout, retry, and rollback counts remain zero; canonical detail is the external-evidence reconciliation authority.
- `CP1-A-C1-PRODUCTION-ACTIVATION-READ-PROOF-RETRY-1` is reviewable from exact base `918ba6b3646baa40965a6b22f475159b7dd7e90f`. Local Wrangler availability is now preflighted separately, but the single sanitized Cloudflare inspection still stopped on its first control-plane read at `cloudflare-deployment-status-control-plane-unavailable`. Exact deployed revision and remote binding/Container presence remain unproven; activation, production C1 read, Checkout, rollback, and cleanup counts remain zero. Canonical detail is the external-evidence reconciliation authority.
