...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-credential-reference-surface-source-final-gate` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-credential-reference-surface-source-final-gate`
   - seed:
   ...
   - current slice target:
     - `credential reference surface source final gate` を PR #302 (`[codex] Recheck credential source approval after PR301`) merge 後の follow-up として確認する。
     - 前提 PR #302 は 2026-06-03 に `codex/comment-translator-preview` へ merge 済みで、merge commit `7b08186833350e21814385fa7294e90c36b919de` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     - PR #300 / #301 merge 前提も維持し、merge commit `a4c272817bab3234eb7a360331c7b54ea419e1b9` / `9b0a3e518262dee4058dca4154a888fe079f48cc` が current branch に含まれることを contract で確認する。
     ...
     - そのため status action の UI wiring、新規 client payload source、localStorage、IndexedDB、sessionStorage、existing handoff payload は実装しない。
     - `scripts/comment-translator-youtube-credential-reference-surface-source-recheck-contract.mjs` と `lib/comment-translator-youtube-client-safe-credential-reference-source.ts` の final gate helper で、PR #302 merge 後も source と evidence が両方揃わない状態を `blocked-pr302-final-gate-missing-surfaced-source-or-approval-evidence` として扱う。
