...
assert.match(beginCheckoutSource, /v_hold_id := gen_random_uuid\(\)[\s\S]+?v_idempotency_key := 'ct-paid-checkout-' \|\| v_hold_id::text[\s\S]+?v_checkout_expires_at_target := p_now \+ interval '31 minutes'/i, "Checkout begin parser derives key and target from hold ID and statement time");
