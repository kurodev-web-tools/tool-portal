...
1. Kuro Live Comment Translator preview branch
   ...
   - immediate next condition: run verification for this request-filter fix, then commit/push/draft PR if checks pass. After merge, rerun the approved target lookup command once; do not run polling until target lookup succeeds with presence-only evidence.
   - next PR candidate: after this filter fix PR, Live Chat target lookup execution evidence rerun PR if sanitized result succeeds. Success must still keep `liveChatId` value out of PR body / docs / command output and hand off only presence-only evidence to the polling command. Then Live Chat polling smoke execution evidence PR.
   ...
   - verification: target lookup command contract, polling smoke command contract, `npm run lint`, `npx tsc --noEmit`, `npm run build`, and `git diff --check` passed after the request-filter fix.
