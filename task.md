...
1. Kuro Live Comment Translator public release roadmap
   ...
   - current PR scope: Task 4 YouTube OAuth server-only credential persistence wiring to Supabase token store, no live connect execution. This PR targets `codex/comment-translator-youtube-oauth-integration`; it wires valid callback handling to a server-only authorization-code exchange adapter, opaque credential reference creation, token-material reference sealing, and trusted Supabase token-store upsert with sanitized disabled/env/exchange/persistence failure states.
   ...
   - current feature branch: `codex/comment-translator-youtube-oauth-token-store-persistence-wiring`, targeting `codex/comment-translator-youtube-oauth-integration`.
   ...
   - next safe action: after Task 4 PR merges into `codex/comment-translator-youtube-oauth-integration`, start Task 5 account integrations status wiring from trusted credential status in a fresh worktree / feature branch. Do not run Google OAuth live connect, YouTube OAuth live connect, live authorization code exchange, live token persistence, provider target lookup, liveChatId lookup, session start smoke, deploy/upload, remote mutation, remote schema migration, Stripe live-mode action, Customer Portal redirect, webhook registration, translation provider API execution, or live/provider execution without same-thread ready preflight, sanitized output review, and explicit approval.
