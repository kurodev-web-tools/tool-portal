...
1. Kuro Live Comment Translator public release roadmap
   - status: preview runtime-smoke-to-operator-UI chain is complete through Task 7, Public Release Roadmap Task 1-15 are merged, Task 16 final QA/readiness PR #419 is merged, and Task 17 private launch gate PR #420 is merged. PR #420 merge commit `f410614c1437fc8ddc95b8f63b18e4f9e7f27cd5` is contained in `origin/codex/comment-translator-preview`. GitHub PR metadata was also checked: PR #420 is `MERGED`, base `codex/comment-translator-preview`, head `codex/comment-translator-private-launch-access-gate-post-pr419`, merged at `2026-06-11T13:35:25Z`; check rollup was Cloudflare Pages `FAILURE`, treated as the known non-main Pages build posture rather than a fresh local regression by itself.
   - current PR scope: Pre-Main Launch Hardening Roadmap Task 18, Operator UX readiness polish.
   ...
   - inspected surfaces for Task 18: `task.md`, `docs/mockups/comment-translator`, `/tools/comment-translator` UI, `/account/billing`, `/account/integrations`, `/account` entry copy, `lib/comment-translator-billing-runtime.ts`, `lib/comment-translator.ts`, and comment-translator UI/action contract scripts.
   - completed in this branch: added a review-only Task 18 HTML mock under `docs/mockups/comment-translator/operator-ux-readiness-task18-mock.html`, then implemented the approved direction in app UI. `/account/billing` now shows Free / Kuro Stream Kit Pro monthly / yearly comparison cards with Free and Pro limits on the same axes, yearly cheaper than 12 monthly payments, price/content advance notice copy, and explicit note that monthly/yearly Stripe readiness is later. `/tools/comment-translator` now shows an explicit Start blocked panel and `/account/integrations` CTA when YouTube credential readiness is not available, and disables Start in that state. `/account/integrations` and `/account` copy now point operators to the right repair and plan surfaces.
   - unchanged in this branch: no deploy/upload, live/provider execution, Stripe live-mode action, new Stripe Price creation, Customer Portal redirect, webhook registration, billing setting mutation, remote mutation, schema migration, provider target lookup, browser storage write, handoff payload expansion, raw comment logging, provider credential exposure, or provider target value exposure was added or run.
   - verification for this branch: RED check for `node scripts/comment-translator-operator-ux-readiness-polish-contract.mjs` failed before implementation because the Task 18 operator UX readiness contract was not exported; GREEN passed after implementation. Related contracts passed: `node scripts/comment-translator-stripe-paid-plan-integration-contract.mjs`, `node scripts/comment-translator-account-integrations-entry-contract.mjs`, and `node scripts/comment-translator-public-operator-session-ui-contract.mjs`. `npm run lint`, `npx tsc --noEmit`, and `npm run build` passed. `npm run build` retained existing non-blocking warnings about deprecated `middleware`, skipped static-export RSC aliases for server-runtime build, and webpack cache serialization.
   - route render checks: local dev server `http://127.0.0.1:3228` rendered `/tools/comment-translator`, `/account/integrations`, and `/account/billing` without framework overlay or browser console warn/error logs. In this unauthenticated local context the private launch gate rendered instead of allowed-tester Task 18 UI, so allowed-tester route rendering remains operator-local/account-dependent.
   - width verification: Codex in-app Browser checks for `/tools/comment-translator`, `/account/integrations`, and `/account/billing` at `390 / 820 / 1024 / 1280 / 1366px` found no horizontal overflow in the private-gated local route state. The approved Task 18 HTML mock was also checked at `390 / 820 / 1024 / 1280 / 1366px`; it showed Kuro Stream Kit Pro, Start blocked guidance, portal teal primary `#008a84`, no horizontal overflow, and no browser console warn/error logs.
   - public-release capable: no. Operator UX readiness is improved for private-gated testing, but provider/cost policy finalization, Stripe live readiness, abuse protection, durable persistence decisions, monitoring, final security/privacy review, private-gated live/provider smoke, production/custom smoke, and final public launch gate flip remain incomplete or approval-gated.
   - residual risk: actual allowed-tester route/browser verification still requires an authenticated allowed tester with operator-local private launch allowlist configuration. Live/provider execution and Stripe live-mode actions remain separately approval-gated.
   - next safe action: create this Task 18 implementation PR targeting `codex/comment-translator-preview`, then proceed to Task 19 Translation provider and cost policy finalization as a separate PR after merge.
