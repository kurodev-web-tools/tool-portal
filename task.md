...
| --- | --- | --- | --- |
| P0-maintenance | Comment Translator Creator current-state roadmap reconciliation | PR #751 までの merged state を task board と architecture authority へ反映し、古い `NC-F1 next` 表示を廃止する docs-only task。runtime、schema、UI、external operation、deploy、activation は変更しない。 | `docs/active/COMMENT_TRANSLATOR_CREATOR_NO_CONTAINER_IMPLEMENTATION_TASK_BOARD.md`, `docs/active/COMMENT_TRANSLATOR_CREATOR_NO_CONTAINER_ARCHITECTURE.md` |
| P1-selection | Comment Translator Creator next implementation | NC-F1 から NC-Q1 までの implementation chain は merged。NC-R1 control plane も merged だが Paid launch readiness は paused NO-GO。次の implementation lane は owner selection required で、候補提示は approval ではない。 | `docs/active/COMMENT_TRANSLATOR_CREATOR_NO_CONTAINER_IMPLEMENTATION_TASK_BOARD.md` |
...
current_branch=codex/comment-translator-current-task-roadmap-reconciliation
...
current_pr=pending-create
current_pr_state=local-docs-only
previous_pr=751
previous_pr_state=merged
previous_pr_final_head=82c075ec8a4153e9a08a3a6b35f3c3f8a13c6fd8
previous_pr_merge_integration_tip=8c86200d915a792488b61a535fb895da88d61f57
...
candidate_lanes_authorization=none-selection-required
...
dependencies=absent-no-install-required-for-docs-only-task
```
...
次の候補は merged dependency graph から選択可能な範囲を示すだけであり、implementation approval ではない。
...
| NC-X4 | overlay templates | exact template scope、UI / width QA boundary |
...
NC-X1 は NC-L1 または別の明示的 post-MVP approval が必要。NC-X8 / NC-X9 は current paused readiness と追加 product decision に依存する。次 task は上表から1件を選び、`1 reviewable goal = 1 PR` とする。
...
- UI / CSS change がある task だけ width QA を行う。この docs-only task では width QA は N/A。
