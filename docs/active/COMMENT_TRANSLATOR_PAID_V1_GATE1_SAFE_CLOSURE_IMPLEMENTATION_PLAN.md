...
## 2026-09-13 local acceptance — approved closed pre-arm deployment transition

The user approved the retry5 draft's bounded repair proposal and local implementation/verification after the published verifier rejected retry4's final disabled version and retained policy. The original design document and retry2 UNKNOWN evidence remain unchanged. This addendum explicitly extends the former single-adapter scope; it does not authorize Git publication or another external allocation.

- Register only `GATE1_RETRY4_PREARM_20260912_V1` / `PREARM_CLIENT_START_FAILED_SAFE_CLOSED`. Validate the complete245-file directory (243 evidence plus receipt/index), four protected backups, four diagnostic files and139 archived static inputs. Original static paths are metadata only; archived objects are read by their SHA256 inside an explicitly supplied root. Reject links, missing/unindexed files, unexpected directories, changed bytes, duplicate paths and oversized inputs.
- Bind the original retained proof, prior allocation closure, approved static manifest/source/producers,17 helper starts and16 successful completions, absent client/observer/arm, zero new controller POST/Hosted operations and exact original canonical state. Revalidate full content/security, native/API and metadata, upload/version/namespace/policy/secret sequence through the final post-secret-deletion disabled version, complete PAT3 revocations, OAuth closure and owned resources0. Validate the historical fresh proof/grant at its saved timestamp without granting or renewing historical execution authority.
- Return a frozen, module-branded closed-deployment proof associated with the exact original retained proof. Preserve the old UNKNOWN, original predecessor, Worker namespace and existing grant/transaction semantics. Summary JSON, cloned/foreign proofs and unregistered formats cannot select the new route.
- Require explicit V2 manifest/approval/index and controller producer. Bind closure evidence in the manifest and controller receipt, the closed receipt in approval, and approval time after closure. Match current disabled version/policy to this verified closure. Reject the closed allocation's unused run ID. Retain all original full-content/native/API/metadata checks, oldest-observation five-minute expiry and one-use grant rules. Fresh controller evidence must include both old PAT2 and previous-allocation PAT3 revocations and zero resources for both allocations.
- Integrate and exercise allocation → V2 index → grant → client read using the actual packet helper in inert fixtures. The local candidate keeps an unconditional `RETRY5_PUBLICATION_REJECTED` entry guard and `sourceCommit: null` while unpublished. Future publication must repin the exact source/Git identity; future external authorization must explicitly bound the additional three revocation GETs (independent Management GET ceiling26).

The primary implements and reviews directly in `D:/V_streamer_tools/.worktrees/gate1-controller-provider-repair-20260912`, branch `codex/gate1-safe-closure-live-preparation-20260912`, base `0c0cf90`. No subagents, model/effort changes, dependency changes, provider/DB/deploy operations or real-use grants. Local evidence and acceptance status are recorded in `.tmp/gate1-deployment-transition-repair-20260912/`; the unpublished integration candidate is `.tmp/gate1-live-execution-20260912-retry5-local/`. Formal stopping and Hosted acceptance remain separate and unaccepted.

Result: **LOCAL_IMPLEMENTATION_ACCEPTED, UNPUBLISHED**. Final263 tests across20 files passed, including the original131 checks and actual packet integration, with fail0/skip0. Changed JavaScript11 syntax, ESLint11 with zero warnings, actual retained-evidence replay and primary diff review passed. The registered directory profile permits only the known empty Wrangler cache alongside normal packet directories. The150-input local candidate remains non-executable until separately authorized publication and a new execution plan. Result/evidence pins are in `.tmp/gate1-deployment-transition-repair-20260912/local-result.json`; Gate1 remains NO-GO.
