...
| Bounded polling | Bounded polling session runtime exists with deterministic adapter, server-only cursor/liveChatId, min interval, empty backoff, and terminal stop mapping. Route wiring is explicitly not wired. | foundation-only | F7 must wire durable active-session polling loop/ticks and quota/budget accounting. |
...
| F7 | Bounded polling is not wired as active-session runtime. | Deterministic bounded polling runtime exists; route wiring is not implemented. | Active-session-only polling ticks, server-only nextPageToken/liveChatId, capped retries, empty chat handling, quota/budget stop. |
...
- YouTube API: `liveChatMessages.list` polling foundation exists, but active-session loop, cursor persistence, quota estimates, retry/backoff, empty-chat behavior, and stop propagation are not public-wired.
