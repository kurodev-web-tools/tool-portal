...
- Current branch: `codex/supabase-stale-contract-cleanup`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- Scope: Stale DB/Supabase/durable contract cleanup for the Free public beta integration line. Retarget old `task.md` handoff marker, stale branch marker, and stale changed-file allowlist assertions to current durable/Supabase invariants without changing Supabase default privileges guard behavior, migrations, RLS/grant checks, or runtime authorization.
- Non-actions: Supabase default privileges guard migration/contract behavior changes, remote read-only Supabase posture check, remote migration apply, Supabase remote mutation, deploy/upload, public gate flip, live/provider execution, OAuth live flow, Google target lookup, Stripe/billing mutation, Checkout/Portal redirect, webhook registration, rate-limit mutation actions, Private launch access gate behavior, OBS and moderation controls: not implemented / not-run.
...
9. `Stale Supabase contract task.md assertion cleanup`: current branch; remove verification noise from old Supabase/durable `task.md` handoff markers and stale changed-file allowlists without touching default privileges guard behavior or running remote posture checks.
10. `OBS Dock display-name policy`: defer until the OBS Dock implementation slice; decide display-name on/off setting, compact layout, and stream-safe visibility there.
