...
- Current branch: `codex/comment-translator-public-ui-cleanup`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- Scope: Public UI cleanup for Free beta launch readiness. The normal operator UI hides internal skipped diagnostics, replaces cache-hit/cache-miss wording with localized user-facing reused/new translation labels, consolidates Free beta usage into one authoritative right-side status area, keeps the left panel focused on Start/Stop/readiness, and reduces F14 Creator locked cards / waitlist / click tracking into one compact Creator closed beta panel. Server quota authority, translation filtering, source/target language controls, Stop retention, Clear preview, author display names, live/provider execution, deploys, migrations, Stripe, OBS, moderation controls, and public gate flips stay out of scope.
...
| P1 | Data deletion / retention / source attribution panel | Kept as sanitized trust/legal information but moved out of the left operations panel into the details area so Start/Stop/readiness stays focused. |
...
1. `Public UI cleanup`: current branch; skipped/cache wording, Today panel consolidation, retention/source panel placement, diagnostics removal from normal UI, and compact Creator closed beta panel.
2. `OBS Dock display-name policy`: defer until the OBS Dock implementation slice; decide display-name on/off setting, compact layout, and stream-safe visibility there.
