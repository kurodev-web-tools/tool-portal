...
import { runWaitlistLocalCases } from './fixtures/comment-translator-paid-core-v1-gate1-waitlist-local-cases.mjs';
...
function row(db){const s=JSON.parse(sql(db,`BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY; SET LOCAL statement_timeout=10000; SET LOCAL row_security=off; SET LOCAL search_path=pg_catalog,public; SELECT ${CURRENT_BACKUP_STATE_SQL}; ROLLBACK;`));s.phase=finalPhase;validateBackupSourceState(s);return s;}
...
function catalogEntry(request,backupPhase,{kind="Catalog",database="gate1_source"}={}){
 assert.ok(/^gate1_(source|wl_[a-z_]+)$/.test(database));
 const shim=path.join(out,'psql-local-'+database+'.exe'),source=path.join(out,'psql-local-'+database+'.cs');
...
class LocalPsql { static int Main(string[] args) {
 ...
 p.StartInfo.EnvironmentVariables["PGHOSTADDR"]="127.0.0.1";p.StartInfo.EnvironmentVariables["PGPORT"]="${port}";p.StartInfo.EnvironmentVariables["PGDATABASE"]="${database}";
 ...
 const destination=privateRoot+'/catalog-'+backupPhase+'-'+database+'-'+kind;
 const r=native('C:/Program Files/PowerShell/7/pwsh.exe',['-NoProfile','-NonInteractive','-File','scripts/comment-translator-paid-core-v1-gate1-catalog-acquire-readonly.ps1','-Environment','Production','-DestinationRoot',destination,'-ProtectedStdin','-BackupPhase',backupPhase,'-ReadbackKind',kind,'-PsqlPath',shim],{input:JSON.stringify(request),timeout:330000,allow:true});
 let receipt;try{receipt=JSON.parse(r.stdout);}catch{throw Error('CATALOG_FIXTURE_OUTPUT_INVALID');}
 fs.writeFileSync(path.join(out,'catalog-'+backupPhase+'-'+database+'-'+kind+'-result.json'),JSON.stringify(receipt,null,2)+'\n');
 if(kind==='WaitlistChecks'){assert.equal(r.status,0,receipt.reason);assert.equal(receipt.status,'WAITLIST_COUNTS_ACQUIRED');return receipt;}
 ...
 assert.equal(artifact.initialReleaseSupplement.archiveSchemaCount,backupPhase!=='pre22'?1:0);
...
try{
 assert.ok(['--local-only','--catalog-only','--post57'].includes(process.argv[2]));assert.equal(process.platform,'win32');
 ...
 let waitlistCases=[];
 if(post57){phase='waitlist_cases';waitlistCases=runWaitlistLocalCases({sql,managedSql:managed,entry:database=>catalogEntry(catalogRequest,'pre22',{kind:'WaitlistChecks',database})});fs.writeFileSync(path.join(out,'waitlist-cases.json'),JSON.stringify(waitlistCases,null,2));}
 phase='source_ddl';
 ...
 if(post57)pending.push(BACKUP_HISTORIES.post57.at(-1));
