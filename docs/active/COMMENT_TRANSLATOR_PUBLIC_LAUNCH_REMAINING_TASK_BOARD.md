...
The PL-G6 public access change preflight is a documentation/contract slice only. `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G6_PUBLIC_ACCESS_CHANGE_PREFLIGHT.md` records the exact same-thread approval surface and keeps `public_release_capable=no`, `pl_g6_public_access_change_status=not-run-approval-gated`, public gate flip, deploy/upload, production env apply, production smoke, and main promotion not-run. The PL-G6C update prepares production/main-domain env readiness and smoke approval text without applying env vars or running smoke.
...
- PL-G6 public access change preflight: complete for docs/contract guidance only. The preflight doc is `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G6_PUBLIC_ACCESS_CHANGE_PREFLIGHT.md`; it records exact same-thread approval text and operation boundaries while keeping `public_release_capable=no`, `pl_g6_public_access_change_status=not-run-approval-gated`, public gate flip, deploy/upload, production env apply, production smoke, and main promotion not-run. PL-G6C now records sanitized operator instructions for production/main-domain env readiness and exact approval text for env readiness confirmation or smoke. No Cloudflare mutation, deploy/upload, production env apply, public gate flip, public access change, production/main-domain smoke, live/provider execution, OAuth live flow, Google target lookup, Supabase query/mutation/migration, Stripe live action, paid entitlement runtime, OBS overlay route/token runtime, or main promotion was run.
