...
1. Kuro Live Comment Translator public release roadmap
   - status: preview runtime-smoke-to-operator-UI chain is complete through Task 7, Public Release Roadmap Task 1-15 are merged, Pre-Main Launch Hardening Roadmap Task 17 PR #420, Task 18 PR #421, Task 19 PR #422, Task 20 PR #423, Task 21 PR #424, Task 22 PR #425, Task 23 PR #426, and Task 24 PR #427 are merged. PR #427 merge commit `ee09174fb79b8323a62c93c32e8d1b4f3cb59c72` is contained in `origin/codex/comment-translator-preview`. GitHub PR metadata was also checked: PR #427 is `MERGED`, base `codex/comment-translator-preview`, head `codex/comment-translator-monitoring-incident-readiness-post-pr426`, merged at `2026-06-12T07:32:59Z`; check rollup was Cloudflare Pages `FAILURE`, treated as the known non-main Pages build posture rather than a fresh local regression by itself.
   - current PR scope: Pre-Main Launch Hardening Roadmap Task 25, provider terms, privacy, and legal copy refresh.
   ...
   - inspected surfaces for Task 25: `task.md`, `lib/legal-content.ts`, `app/terms/page.tsx`, `app/privacy/page.tsx`, `app/legal/tokushoho/page.tsx`, `components/legal/LegalDocumentPage.tsx`, `app/tools/comment-translator/page.tsx`, `components/comment-translator/CommentTranslatorDock.tsx`, `lib/comment-translator.ts`, `app/account/integrations/page.tsx`, `components/account/AccountIntegrationsShell.tsx`, `app/account/billing/page.tsx`, `components/account/AccountBillingShell.tsx`, `components/comment-translator/CommentTranslatorPrivateLaunchUnavailable.tsx`, `docs/active/COMMENT_TRANSLATOR_PROVIDER_COST_POLICY.md`, `docs/active/COMMENT_TRANSLATOR_STRIPE_LIVE_READINESS.md`, `docs/active/COMMENT_TRANSLATOR_MONITORING_INCIDENT_READINESS.md`, `lib/comment-translator-provider-policy-runtime.ts`, `lib/comment-translator-billing-runtime.ts`, and related Task 20/21/24 contract scripts.
   - completed in this branch: added `docs/active/COMMENT_TRANSLATOR_PROVIDER_LEGAL_COPY_REFRESH.md` and `scripts/comment-translator-provider-legal-copy-refresh-contract.mjs`; refreshed `/terms`, `/privacy`, `/legal/tokushoho`, `/tools/comment-translator`, `/account/integrations`, `/account/billing`, and private-launch copy to state the final provider policy. Free routes to Azure Translator, Paid routes to OpenAI mini, Paid recoverable provider errors may use Azure fallback, and DeepL / Gemini Flash/Lite / Cloudflare Workers AI remain comparison-only. Legal/privacy copy now states provider processing, retention/training posture, support/contact path, and paid-plan/Stripe approval-gated wording without exposing sensitive metadata.
   - unchanged in this branch: no provider routing runtime behavior, SQL migration file, RLS policy, remote Supabase migration apply, remote mutation, remote alert/dashboard mutation, Stripe live-mode action, Product/Price creation, Checkout execution, Customer Portal redirect, webhook registration, billing setting mutation, deploy/upload, live/provider execution, provider target lookup, liveChatId lookup, browser storage expansion, handoff payload expansion, raw comment logging, credential value exposure, private provider target value exposure, quota write, durable persistence, or handoff payload change was added or run.
   - verification for this branch: `npm ci --prefer-offline` succeeded. RED `node scripts/comment-translator-provider-legal-copy-refresh-contract.mjs` failed before implementation because Task 25 provider legal/copy text was missing; GREEN passed after implementation. Route render checks passed on local dev server `127.0.0.1:3057`: `/terms/`, `/privacy/`, `/legal/tokushoho/`, `/tools/comment-translator/`, `/account/integrations/`, and `/account/billing/` returned 200 and contained provider/legal/private-launch copy. In-app browser checks also confirmed `/privacy/`, `/terms/`, `/legal/tokushoho/`, `/tools/comment-translator/`, `/account/integrations/`, and `/account/billing/` had no sensitive-value pattern and no horizontal overflow at the default 1270-1280px viewport. Changed-files no-secret scan passed. `npm run lint`, `npx tsc --noEmit`, `npm run build`, and `git diff --check` passed.
   - width checks skipped: Task 25 changed only copy within existing legal/account/tool/private-launch layouts, a docs evidence file, a Node contract script, and this task note. There was no visible UI/CSS/layout change, and no CSS, layout structure, responsive breakpoint, storage, or browser-state behavior changed; route render checks still cover the visible surfaces.
   - public-release capable: no. Provider legal/copy surfaces are aligned with the final provider policy, but final security/privacy review, private-gated live/provider smoke, private-gated main promotion/production smoke, and final public launch gate flip remain incomplete or approval-gated.
   - residual risk: provider terms, pricing, training, and retention posture must be rechecked against operator account/dashboard evidence before live/provider execution or paid launch. Stripe live-mode/dashboard actions, remote schema/migration apply, deploy/upload, live/provider execution, provider/billing dashboard checks, and deployed smoke were not run.
   - next safe action: create this Task 25 legal/copy refresh PR targeting `codex/comment-translator-preview`, then proceed to Task 26 security and privacy final review as a separate PR after merge.
