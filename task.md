...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-credential-status-display-source-intake-gate` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-credential-status-display-source-intake-gate`
   - seed:
   ...
   - current slice target:
     - `source/evidence intake gate` を PR #303 (`[codex] Recheck credential source final gate after PR302`) merge 後の follow-up として確認する。
     - 前提 PR #303 は 2026-06-03 に `codex/comment-translator-preview` へ merge 済みで、merge commit `1c82713dbeea54ffe0990415618adbb5d8ee55b2` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     - PR #300 / #301 / #302 merge 前提も維持し、merge commit `a4c272817bab3234eb7a360331c7b54ea419e1b9` / `9b0a3e518262dee4058dca4154a888fe079f48cc` / `7b08186833350e21814385fa7294e90c36b919de` が current branch に含まれることを contract で確認する。
     ...
     - `scripts/comment-translator-youtube-credential-reference-surface-source-recheck-contract.mjs` と `lib/comment-translator-youtube-client-safe-credential-reference-source.ts` の PR #303 intake gate helper で、PR #303 merge 後も source と evidence が両方揃わない状態を `blocked-pr303-intake-gate-missing-surfaced-source-or-approval-evidence` として扱う。
