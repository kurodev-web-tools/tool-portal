...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-credential-reference-surface-source-approval` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-credential-reference-surface-source-approval`
   - seed:
   ...
   - current slice target:
     - `credential reference surface source and approval evidence recheck` を PR #301 (`[codex] Recheck YouTube credential reference surface source`) merge 後の follow-up として確認する。
     - 前提 PR #301 は 2026-06-03 に `codex/comment-translator-preview` へ merge 済みで、merge commit `9b0a3e518262dee4058dca4154a888fe079f48cc` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     - PR #300 merge 前提も維持し、merge commit `a4c272817bab3234eb7a360331c7b54ea419e1b9` が current branch に含まれることを contract で確認する。
     ...
     - `scripts/comment-translator-youtube-credential-reference-surface-source-recheck-contract.mjs` と `lib/comment-translator-youtube-client-safe-credential-reference-source.ts` の recheck helper で、PR #301 merge 後も source と evidence が両方揃わない状態を `blocked-pr301-follow-up-missing-surfaced-source-or-approval-evidence` として扱う。
