...
const expectedBackground = `${phase5BackgroundPrefix}x-announcement-background-v1.png`;
...
const expectedDecorationFiles = [
  ...
  "x-announcement-label-plaque-ivory-candidate-uniform-cell.png"
];
...
assert.ok(xAnnouncementPreset, "x_announcement preset exists");

const background = xAnnouncementPreset.layers.find((layer) => layer.type === "image" && layer.name.includes("背景"));
assert.ok(background, "x_announcement has a background image layer");
assert.equal(background.src, expectedBackground, "x_announcement uses the phase 5 generated background");
assert.equal(background.locked, true, "x_announcement phase 5 background is locked by default");
const backgroundPath = path.join(root, "public", expectedBackground.replace(/^\//, ""));
assert.equal(fs.existsSync(backgroundPath), true, "x_announcement phase 5 generated background exists");
assert.deepEqual(readPngSize(backgroundPath), { width: 1280, height: 720 }, "x_announcement background uses the app canvas size");
assert.ok(fs.statSync(backgroundPath).size < 1_800_000, "x_announcement phase 5 background stays reasonably lightweight");
...
for (const fileName of expectedDecorationFiles.slice(0, 4)) {
  assert.equal(decorationSources.has(fileName), true, `x_announcement preset uses ${fileName}`);
}
assert.equal(
  decorationSources.has("x-announcement-label-plaque-ivory-candidate-uniform-cell.png"),
  false,
  "x_announcement keeps the alternate ivory label as an unused candidate asset"
);

assert.equal(
  xAnnouncementPreset.layers.some((layer) => layer.type === "image" && layer.src.endsWith("/phase4/x-post-card-base.svg")),
  false,
  "phase 5 x_announcement burns the large post card panel into the generated background for MVP"
);

for (const roleName of ["見出し", "時刻", "サブ", "ラベル"]) {
  assert.ok(
    xAnnouncementPreset.layers.some((layer) => layer.type === "text" && layer.name.includes(roleName)),
    `x_announcement keeps editable ${roleName} text layer`
  );
}

for (const editableShapeName of ["本文罫線", "サブ下ライン", "立ち絵guide枠"]) {
  assert.ok(
    xAnnouncementPreset.layers.some((layer) => layer.type === "shape" && layer.name.includes(editableShapeName)),
    `x_announcement keeps editable ${editableShapeName} as a shape layer`
  );
}

const draft = lib.createDraftFromPreset("x_announcement");
...
assert.equal(
  ...
  expectedBackground,
  "phase 5 x_announcement background survives draft normalization"
);

console.log("thumbnail phase 5 x_announcement preset contract checks passed");
