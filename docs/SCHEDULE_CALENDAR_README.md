...
長文入力による保存 payload、投稿補助プレビュー、handoff payload の肥大化を避けるため、予定タイトルは `120` 文字、予定ごとの告知文は `1200` 文字、予定 / テンプレートのハッシュタグは `300` 文字、テンプレート本文は `2000` 文字を上限にします。
既存 / 旧形式データの読み込み時も同じ上限へ丸めますが、保存 payload version は `2` のままです。
...
Schedule Calendar 由来ではメイン画像は未選択のため、SNS分割画像メーカー側ではまずメイン画像を選んでから個別PNG/JPEGを書き出します。
...
Thumbnail Editor 由来では受け取った画像を確認してから、必要な追加画像や境界調整を行い、個別PNG/JPEGとして保存します。
...
- freeze 前の現行対象は、予定管理、投稿補助、予定一覧、設定、バックアップ / 復元、ブラウザ内 handoff の安定確認に限定します。
- Schedule Calendar 由来の handoff は、Thumbnail Editor / SNS分割画像メーカーへ予定テキスト、日時、カテゴリ、プラットフォーム、告知文、ハッシュタグなどのテキスト metadata だけを渡します。画像本体は渡しません。
- 週間予定画像の直接生成、Google Calendar 連携、ログイン / サーバー同期、例外日、繰り返し終了日、シリーズ一括編集は freeze 後候補として扱います。
- UI / 表示文言を変更した場合のみ、`390 / 820 / 1024 / 1280 / 1366px` の幅別確認を残します。
