...
export type CommentTranslatorCreatorObsOverlayBrowserSessionRecord = {
...
export interface CommentTranslatorCreatorObsOverlayBrowserSessionStore {
  readByDigest(capabilityDigest: string): Promise<
    | { readonly status: "ready"; readonly record: CommentTranslatorCreatorObsOverlayBrowserSessionRecord }
  ...
  redeemAndWriteCurrent(request: {
    ...
    | { readonly status: "redeemed"; readonly record: CommentTranslatorCreatorObsOverlayBrowserSessionRecord }
    | { readonly status: "denied"; readonly reason: "invalid-token" | "stale-or-replayed-token" }
...
export type CommentTranslatorCreatorObsOverlayBrowserSessionSupabaseClient = {
  rpc(functionName: string, parameters: Record<string, unknown>): Promise<SupabaseRpcResult>;
};

export const commentTranslatorCreatorObsOverlayBrowserSessionStoreContract = {
  implementationStage: "nc-o2-local-obs-overlay-browser-session-store",
  runtime: "server-only",
  tableName: "comment_translator_creator_obs_overlay_browser_sessions",
  rowAccess: "trusted-server-service-role-rpc-only",
  persistence: "sha256-capability-digest-only",
  currentAuthority: "owner-session-token-version-bound",
  redemption: "atomic-nc-o1-redeem-and-browser-capability-replacement",
  creatorActivation: "fixed-closed",
  remoteSupabaseMigrationApply: "not-run-in-this-thread"
} as const;

export function createTrustedCommentTranslatorCreatorObsOverlayBrowserSessionStore({
  env,
  createSupabaseClient = createTrustedSupabaseServiceRoleClient
}: {
  readonly env?: Partial<Record<TrustedEnvName, string | undefined>>;
  readonly createSupabaseClient?: (url: string, serviceRoleKey: string) => CommentTranslatorCreatorObsOverlayBrowserSessionSupabaseClient;
} = {}):
  | { readonly status: "ready"; readonly store: CommentTranslatorCreatorObsOverlayBrowserSessionStore }
  ...
  return {
    status: "ready",
    store: createCommentTranslatorCreatorObsOverlayBrowserSessionStore({
      supabase: createSupabaseClient(url, serviceRoleKey)
    })
  };
}

export function createCommentTranslatorCreatorObsOverlayBrowserSessionStore({
  supabase
}: {
  readonly supabase: CommentTranslatorCreatorObsOverlayBrowserSessionSupabaseClient;
}): CommentTranslatorCreatorObsOverlayBrowserSessionStore {
  return {
    async readByDigest(capabilityDigest) {
      const result = await callRpc(supabase, "read_comment_translator_creator_obs_overlay_browser_session", {
    ...
    async redeemAndWriteCurrent({ tokenDigest, capabilityDigest, nowIso }) {
      const result = await callRpc(supabase, "redeem_and_write_comment_translator_creator_obs_overlay_browser_session", {
      ...
      if (result.data.status === "denied") {
        return result.data.reason === "stale-or-replayed-token"
          ? { status: "denied", reason: "stale-or-replayed-token" }
...
async function callRpc(
  supabase: CommentTranslatorCreatorObsOverlayBrowserSessionSupabaseClient,
  functionName: string,
...
function parseRecord(value: unknown): CommentTranslatorCreatorObsOverlayBrowserSessionRecord | null {
...
): CommentTranslatorCreatorObsOverlayBrowserSessionSupabaseClient {
