...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-token-store-status-endpoint-skeleton` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-token-store-status-endpoint-skeleton`
   - seed:
   ...
   - current slice target:
     - `credential status endpoint / server action skeleton` を PR #290 の trusted Supabase adapter / credential sanitized status boundary merge 後の最小 server-only slice として `codex/comment-translator-preview` 宛てに閉じる。
     - 前提 PR #290 (`Add YouTube token store Supabase status adapter skeleton`) は `2026-06-02T11:53:58Z` に `codex/comment-translator-preview` へ merged 済みで、merge commit `cfeb00a0be37dc4765495c10d11e3e5d37d2f259` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     - `lib/comment-translator-youtube-token-store-supabase-adapter.ts`、`scripts/comment-translator-youtube-token-store-supabase-adapter-status-contract.mjs`、関連 token-store contract を確認済み。今回の実装は trusted status read / browser-readable boundary / endpoint skeleton / server action skeleton に限定し、remote Supabase DB migration apply や service role client wiring は行わない。
     - `lib/comment-translator-youtube-token-store-supabase-adapter.ts` に `getCredentialStatus` を追加し、trusted Supabase-like client から `credential_reference_id` で row を読み、返却は既存の sanitized status に限定する。
     - `lib/comment-translator-youtube-credential-status-boundary.ts` は `import "server-only";` で開始し、browser-readable output を credential reference、provider channel id、`youtube.readonly` scope label、scope set、expiresAtIso、expiry status、revoked state、reconnect-required に限定する。encrypted row、ciphertext reference、decrypt capability、service_role key、managed secret value、OAuth access token / refresh token / authorization code value は返さない。
     - `/api/comment-translator/youtube/credential-status` と `getYouTubeOAuthCredentialStatusAction` は skeleton として追加した。現時点では service role client 未接続のため trusted adapter は `null` で、missing / unwired / `YOUTUBE_OAUTH_CREDENTIAL_RESOLUTION_DISABLED` 時も sanitized unavailable / reconnect-required state だけを返す。
     ...
     - 未確認範囲: remote Supabase DB への migration apply、Supabase migration / RLS smoke、service role client wiring、safe live YouTube login / OAuth / owner verification / Live Chat polling smoke、Google API live call、refresh runtime、full revocation runtime。
     - 残リスク: endpoint / server action は skeleton であり、実 service_role client wiring、operational KMS / managed secret resolver、credential owner authorization policy、rotation re-encrypt path、audit / retention writer、safe live smoke は後続 PR。
     - 次 PR 候補は `YouTube token store trusted service role status wiring contract`。ただし remote apply、live OAuth、Google API live call、refresh / revocation 本実装、client UI wiring は別承認のまま維持する。
