...
for (const requiredPath of [wrapperPath, completionDocPath, taskPath]) {
...
assert.equal(providerUnavailablePayload.returnedCount, 3);
assert.equal(providerUnavailablePayload.eligibleCount, 3);
assert.equal(providerUnavailablePayload.providerRequestCount, 3);
assert.equal(providerUnavailablePayload.providerCallCount, 3);
assert.equal(providerUnavailablePayload.translatedCount, 0);
assert.equal(providerUnavailablePayload.skippedCount, 3);
assert.equal(providerUnavailablePayload.languagePolicySkippedCount, 0);
assert.equal(providerUnavailablePayload.perMinuteSkippedCount, 0);
assert.equal(providerUnavailablePayload.providerUnavailableSkippedCount, 3);
assert.equal(providerUnavailablePayload.recoverableErrorCount, 0);
assert.equal(providerUnavailablePayload.terminalErrorCount, 3);
assert.equal(providerUnavailablePayload.sourceAttributionLabel, "unavailable");
assert.equal(providerUnavailablePayload.sourceAttributionAvailabilityLabel, "not-produced-by-provider-harness");
assert.equal(providerUnavailablePayload.pass, true);
...
for (const [label, source] of [
  ...
  for (const fragment of [
    ...
    "contract-first-triage-after-pr535 / diagnostic-boundary-prepared",
    "translated count 0 / skipped count 3",
    "provider request count 3 / provider call count 3",
    "provider-unavailable skipped count 3",
    "terminal error count 3",
    "language-policy skipped count 0 / per-minute skipped count 0",
    "source attribution availability label `not-produced-by-provider-harness`",
    "source attribution label `unavailable`",
    "Start, Stop, target lookup execution, liveChatMessages.list, Azure/OpenAI provider execution, and UI/feed confirmation were not run in this follow-up",
...
console.log("comment translator Free beta PL-G3 diagnostic boundary after PR #535 contract checks passed");
