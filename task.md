...
- Current branch: `codex/comment-translator-creator-waitlist-admin-ui`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- Scope: Creator waitlist admin + public UI trim. The normal public Comment Translator UI removes Manual/Paste input and bottom details/test-input surfaces, keeps normal source/target/comment display controls, replaces the static Creator closed-beta/card click draft with server-owned pre-registration state, and adds an admin-only Creator waitlist list/count page. Server quota authority, translation runtime, source/target behavior, Stop retention, Clear preview, author display names, live/provider execution, deploys, remote migration apply, Stripe, OBS, moderation controls, and public gate flips stay out of scope.
...
2. `Creator waitlist admin + public UI trim`: current branch; removes normal-public manual/paste and bottom details/test-input surfaces, implements durable Creator pre-registration state, and adds the admin-only waitlist page.
