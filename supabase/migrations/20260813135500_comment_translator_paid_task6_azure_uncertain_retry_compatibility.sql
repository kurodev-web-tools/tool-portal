-- Prepare the Task 6 Azure fallback guard for immutable historical replay.
--
-- The immutable 20260813140000 migration replaces two legacy text needles.
-- Replacing only the opening line of the uncertain guard would leave its old
-- RAISE/END IF behind and produce invalid PL/pgSQL on a clean replay. This
-- migration hardens the complete canonical legacy guard first, then places the
-- two historical needles inside an explicit block comment. The historical
-- migration can therefore run in order without changing executable behavior.
-- Complete hardened definitions converge idempotently. Partial, duplicate, or
-- malformed definitions fail closed. Function identity, privileges, ownership,
-- SECURITY DEFINER, search_path, and every provider/data boundary are preserved
-- by executing only the reconstructed definition returned by PostgreSQL.

do $task6_azure_uncertain_retry_compatibility$
declare
  v_definition text;
...
begin
  ...
  if v_valid_marker_count = 0 then
    if (length(v_definition) - length(replace(v_definition, v_anchor, ''))) / length(v_anchor) <> 1 then
      raise exception 'Task 6 Azure compatibility anchor is unreadable';
    end if;
    v_definition := replace(v_definition, v_anchor, v_compatibility_marker || chr(10) || v_anchor);
  end if;
...
$task6_azure_uncertain_retry_compatibility$;
