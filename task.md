...
1. Kuro Live Comment Translator public release roadmap
   ...
   - current PR scope: Task 6 tool credential source wiring away from fixed preview credential reference. This PR targets `codex/comment-translator-youtube-oauth-integration`; `/tools/comment-translator` now reads server-owned trusted credential status metadata and resolves the owner-bound credential reference inside server-only actions/helpers for status/session readiness.
   ...
   - current feature branch: `codex/comment-translator-youtube-oauth-tool-credential-source`, targeting `codex/comment-translator-youtube-oauth-integration`.
   ...
   - unchanged in this branch: no SQL migration file, RLS policy, remote Supabase migration apply, remote mutation, remote alert/dashboard mutation, Stripe live-mode action, Product/Price creation, Checkout execution, Customer Portal redirect, webhook registration, billing setting mutation, manual Cloudflare upload/deploy command, client-readable browser storage expansion, handoff payload expansion, raw comment logging, credential value exposure, private provider target value exposure, quota write, provider target lookup, liveChatId lookup, translation provider API execution, Google OAuth live connect execution, YouTube OAuth live connect execution, live authorization code exchange, live token persistence, provider revoke/live API execution, or live/provider execution is run.
   - verification for this branch: Task 6 RED/GREEN `node scripts/comment-translator-youtube-oauth-tool-credential-source-contract.mjs` was used for the new tool/session readiness boundary; `npm run lint`, `npx tsc --noEmit`, `npm run build`, changed-files no-secret scan, and `git diff --check` passed locally. Existing Task 5/4/3/2 focused contracts were attempted and still validate their historical boundaries, but their task-specific changed-file allowlists are no longer applicable to this Task 6 tool/session slice and fail on expected Task 6 files such as `app/tools/comment-translator/actions.ts`.
   ...
   - next safe action: after Task 6 PR merges into `codex/comment-translator-youtube-oauth-integration`, start Task 7 allowed-tester connection smoke readiness / sanitized evidence in a fresh worktree / feature branch. Do not run Google OAuth live connect, YouTube OAuth live connect, live authorization code exchange, live token persistence, provider target lookup, liveChatId lookup, session start smoke, deploy/upload, remote mutation, remote schema migration, Stripe live-mode action, Customer Portal redirect, webhook registration, translation provider API execution, or live/provider execution without same-thread ready preflight, sanitized output review, and explicit approval.
