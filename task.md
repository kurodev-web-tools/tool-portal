...
11. `Remote Supabase catalog posture read`: current branch; read table/RLS/grant/default-privilege/advisor posture as sanitized count/status/table evidence only. Do not mutate remote state.
12. `OBS Dock display-name policy`: defer until the OBS Dock implementation slice; decide display-name on/off setting, compact layout, and stream-safe visibility there.
