...
const expectedBackground = `${phase5BackgroundPrefix}endurance-stream-background-v1.png`;
const minimumUniformCellPadding = 76;
const expectedDecorationFiles = [
  "endurance-stream-goal-badge-panel-lime-orange-uniform-cell.png",
...
const endurancePreset = lib.thumbnailPresets.find((item) => item.id === "endurance_stream");
assert.ok(endurancePreset, "endurance_stream preset exists");
assert.equal(endurancePreset.name, "耐久配信", "endurance_stream keeps the expected display name");
assert.equal(endurancePreset.category, "配信ジャンル", "endurance_stream stays in the existing streaming category");
assert.equal(endurancePreset.usageLabel, "耐久 / 目標", "endurance_stream uses the scoped usage label");

const background = endurancePreset.layers.find((layer) => layer.type === "image" && layer.name.includes("背景"));
assert.ok(background, "endurance_stream has a background image layer");
assert.equal(background.src, expectedBackground, "endurance_stream uses the generated production background");
assert.equal(background.locked, true, "endurance_stream background is locked by default");
const backgroundPath = path.join(root, "public", expectedBackground.replace(/^\//, ""));
assert.equal(fs.existsSync(backgroundPath), true, "endurance_stream generated background exists");
assert.deepEqual(readPngSize(backgroundPath), { width: 1280, height: 720 }, "endurance_stream background uses the app canvas size");
assert.ok(fs.statSync(backgroundPath).size < 1_800_000, "endurance_stream background stays reasonably lightweight");
...
assert.equal(
  ...
  false,
  "endurance_stream does not depend on phase 4 decoration asset layers"
);
...
for (const [roleName, text] of expectedEditableText) {
  const layer = endurancePreset.layers.find((item) => item.type === "text" && item.name.includes(roleName));
  assert.ok(layer, `endurance_stream keeps editable ${roleName} text layer`);
  assert.equal(layer.text, text, `endurance_stream initial ${roleName} text matches the scoped copy`);
}

for (const editableShapeName of ["立ち絵挿入ガイド", "ラベル横ライン", "見出し下ライン", "時刻下ライン"]) {
  assert.ok(
    endurancePreset.layers.some((layer) => layer.type === "shape" && layer.name.includes(editableShapeName)),
    `endurance_stream keeps editable ${editableShapeName} as a shape layer`
  );
}

assert.ok(
  endurancePreset.layers.some(
    (layer) =>
      layer.type === "image" &&
      layer.src.endsWith("endurance-stream-progress-divider-lime-cyan-orange-uniform-cell.png") &&
      layer.width >= 700
  ),
...
assert.equal(
  ...
  false,
  "endurance_stream is no longer listed as a future preset-body candidate"
);

const draft = lib.createDraftFromPreset("endurance_stream");
...
assert.equal(
  ...
  expectedBackground,
  "endurance_stream background survives draft normalization"
);

console.log("thumbnail endurance_stream usecase preset contract checks passed");
