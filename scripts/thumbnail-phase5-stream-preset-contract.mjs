...
const expectedBackground = `${phase5BackgroundPrefix}stream-announce-background-v1.png`;
...
const streamPreset = lib.thumbnailPresets.find((item) => item.id === "stream_announce");
assert.ok(streamPreset, "stream_announce preset exists");

const background = streamPreset.layers.find((layer) => layer.type === "image" && layer.name.includes("背景"));
assert.ok(background, "stream_announce has a background image layer");
assert.equal(background.src, expectedBackground, "stream_announce uses the phase 5 generated background");
assert.equal(background.locked, true, "stream_announce phase 5 background is locked by default");
const backgroundPath = path.join(root, "public", expectedBackground.replace(/^\//, ""));
assert.equal(fs.existsSync(backgroundPath), true, "stream_announce phase 5 generated background exists");
assert.deepEqual(readPngSize(backgroundPath), { width: 1280, height: 720 }, "stream_announce background uses the app canvas size");
assert.ok(fs.statSync(backgroundPath).size < 1_800_000, "stream_announce phase 5 background stays reasonably lightweight");
...
for (const fileName of expectedDecorationFiles) {
  assert.equal(decorationSources.has(fileName), true, `stream_announce preset uses ${fileName}`);
}

assert.equal(
  streamPreset.layers.some((layer) => layer.type === "image" && layer.src.includes("/decorations/phase4/")),
  false,
  "stream_announce no longer depends on phase 4 decoration asset layers"
);

for (const roleName of ["見出し", "時刻", "サブ", "ラベル"]) {
  assert.ok(
    streamPreset.layers.some((layer) => layer.type === "text" && layer.name.includes(roleName)),
    `stream_announce keeps editable ${roleName} text layer`
  );
}

for (const editableShapeName of ["立ち絵挿入ガイド", "ラベル横ライン", "見出し下ライン", "時刻下ライン"]) {
  assert.ok(
    streamPreset.layers.some((layer) => layer.type === "shape" && layer.name.includes(editableShapeName)),
    `stream_announce keeps editable ${editableShapeName} as a shape layer`
  );
}

assert.ok(
  streamPreset.layers.some(
    (layer) =>
      layer.type === "image" &&
      layer.src.endsWith("stream-standee-frame-glow-uniform-cell.png") &&
      layer.width >= 520 &&
      layer.height >= 680
  ),
  "stream_announce keeps the right standee frame as a large editable asset layer"
);
...
assert.equal(
  ...
  expectedBackground,
  "phase 5 stream_announce background survives draft normalization"
);

console.log("thumbnail phase 5 stream preset contract checks passed");
