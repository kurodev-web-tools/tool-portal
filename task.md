...
- CP1 remaining external-evidence reconciliation is current through reviewed base `76b4b8274bf7b12b142968853d7c7f89ebda01be`. `CP1-A-MIG-C3`, `CP1-A-MIG-C5`, `CP1-A-MIG-C6`, `CP1-A-MIG-C7`, `CP1-A-MIG-C8`, `CP1-A-MIG-C9`, `CP1-A-MIG-C11-RETRY-3`, and `CP1-A-STORE-READINESS` are consumed/pass. Non-C1 store readiness covers C3/C5/C6/C7/C8/C9/C11 with 24 catalog source-bound predicates plus separate Data API exposure and client fail-closed evidence (`26` total / `0` row payload / `0` mutations). Data API exposure and catalog RLS/service-role authority remain separate proof boundaries. Durable sanitized evidence is appended to `docs/active/COMMENT_TRANSLATOR_CREATOR_PAID_CP1_EXTERNAL_EVIDENCE_RECONCILIATION_PREFLIGHT.md`. The next ordered independent unit is `CP1-A-STORE-WRITE-READ`; deployed behavior remains separately approval-gated. C1 remains `disconnected-fail-closed`, with `0/7` candidate proofs and `0` eligible candidates.
