...
export function createRehearsalConfigurationController(transport,target){
 ...
 return {async apply(change){
  ...
  try{
   ...
   phase='patch';const response=await transport.patch(request);if(response?.status!==200)reject();
...
export function createManagedRehearsalConfiguration({approvedRecoveryRef,protectedProjectRefs,managementToken,stop}){
 ...
 const send=(method,url,postData)=>new Promise((resolve,rejectPromise)=>{
  let done=false,request,timer,size=0;const chunks=[];
  const finish=(ok,value)=>{if(done)return;done=true;clearTimeout(timer);if(!ok){chunks.length=0;request?.destroy();rejectPromise(Error('REHEARSAL_CONFIGURATION_HTTP_REJECTED'));}else resolve(value);};
  try{request=https.request(url,{method,headers:{Authorization:'Bearer '+managementToken,'Content-Type':'application/json',...(postData?{'Content-Length':Buffer.byteLength(postData)}:{})}},response=>{
   ...
   response.on('end',()=>{try{if(!response.complete)throw Error();const body=parseStrictJson(Buffer.concat(chunks).toString('utf8'));finish(true,{status:200,body});}catch{finish(false);}});
  });request.on('error',()=>finish(false));timer=setTimeout(()=>finish(false),10000);request.end(postData);}catch{finish(false);}
 });
 return createRehearsalConfigurationController({
  ...
  claim:change=>nativeRehearsalAttemptLedger().claim(rehearsalConfigurationClaimSha256(approvedRecoveryRef,change)),
  patch:request=>{const service=Object.keys(fields).find(service=>urlFor(service,target)===request.url);if(!service||!fullBefore.has(service)||pendingPatches.has(service))reject();pendingPatches.set(service,parseStrictJson(request.postData));return send(request.method,request.url,request.postData);},stop,
 },target);
}
