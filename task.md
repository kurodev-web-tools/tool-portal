...
- PR #688 is merged at current integration tip `b4409937b4ef637f3218c6d24e45a32ef20920ce`; reviewed head `d6cf6e119b9fd987330e3ede53e536ed2c7ddd5d` is contained in integration. S2AP remains consumed/pass and S2AQ remains locally GREEN. The current local follow-up adds a synthetic-only, single-use bridge from the process-owned Buffers to the existing C1 factory-result/store/read shape, with `10/10` focused fixtures GREEN including stop-during-factory/read fail-closed behavior. The real constructor remains disconnected because it requires zero-fill-incompatible immutable string copies; real client initialization and remote/deployed execution remain not-run / approval-gated.
