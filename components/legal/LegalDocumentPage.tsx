    ...
    <article className="mx-auto max-w-4xl">
      <header className="border-b border-border pb-6">
        ...
        <dl className="mt-5 grid gap-2 text-xs text-muted sm:grid-cols-2">
          <div className="rounded-base bg-surface-muted px-3 py-2">
            <dt className="font-bold text-foreground">制定日</dt>
            <dd className="mt-1">{document.effectiveDate}</dd>
          </div>
          <div className="rounded-base bg-surface-muted px-3 py-2">
            <dt className="font-bold text-foreground">最終更新日</dt>
      ...
      <div className="space-y-9 py-8">
        {document.sections.map((section) => (
          <section key={section.heading} aria-labelledby={section.heading} className="space-y-4">
            <h2 id={section.heading} className="text-xl font-bold tracking-tight text-foreground">
              {section.heading}
            </h2>
