...
- Creator C1 containerless billing-state read design PR #718 is merged at `9ad39a530a2394995b031849929d88217b207f5c`. Phase 1 additive source is locally review-ready in the current isolated worktree: one migration, pgTAP/catalog/behavior coverage, and an unconnected fixed-projection application seam preserve the signed-evidence writer ordering and `trialing -> paid-inactive` rule. Repository verification found that schema-local default `REVOKE` cannot override PostgreSQL's global routine defaults, so the migration uses effective global function-`EXECUTE` default revokes for the two reviewed API routine creators while leaving existing objects unchanged. The focused local contract is GREEN; pgTAP execution is unchecked because this worktree has no local Supabase CLI, `psql`, or Docker. Remote apply, owner-binding backfill/count proof, custom-schema exposure/schema-cache work, production consumer cutover, deploy, activation, Checkout, CP2, and public paid launch remain separately approval-gated / not run.
...
- `CP1-A-C1-PRODUCTION-ACTIVATION-READ-PROOF-RETRY-1` is reviewable from exact integration `918ba6b3646baa40965a6b22f475159b7dd7e90f`. Lockfile-faithful local setup separated the prior local-Wrangler ambiguity, then the one approved sanitized inspection stopped on its first control-plane read at `cloudflare-deployment-status-control-plane-unavailable`. Exact deployed revision and remote binding/Container presence remain unproven; activation and production C1 read counts remain zero. Canonical detail is the external-evidence reconciliation authority.
