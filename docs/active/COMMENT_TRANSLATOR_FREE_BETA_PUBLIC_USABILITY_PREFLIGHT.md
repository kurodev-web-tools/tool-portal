...
PL-G2K allowed-tester route/API harness smoke execution: `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G2K_APPROVED_ALLOWED_TESTER_ROUTE_API_HARNESS_SMOKE_EXECUTION_AFTER_PL_G2J.md` rechecks the PL-G2 status route and PL-G2A harness route after PL-G2J with exact approval label present and records keep blocked / blocked-auth-unavailable. The allowed-tester route/API harness smoke remains incomplete because the operator-local sanitized run returned HTTP 403 / stopped / auth-failed for the status route, HTTP 403 / blocked-private-launch-gated / count 0 / pass false for the harness route, and HTTP 403 / private-launch-gated / auth-unavailable / pass false for the value-free private launch diagnostic. Next safe action is to verify deployed `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` readiness as value-free references, then verify the same-origin allowed-tester browser session without printing or storing values.
PL-G3 Start-to-translation smoke evidence: `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G3_START_TO_TRANSLATION_SMOKE.md` records blocked-no-approval for the FB-L4 Start-to-translation smoke execution gate, and `docs/active/COMMENT_TRANSLATOR_FREE_BETA_PL_G3_START_TO_TRANSLATION_SMOKE_EVIDENCE_FOLLOW_UP.md` records the post-PL-G2C recheck as keep blocked / blocked-no-approval. The exact approval label remains `approved-fb-l4-start-to-translation-smoke`; status precheck, explicit Start, server-only live target lookup, one bounded `liveChatMessages.list` polling step, Free Azure translation, UI feed confirmation, usage, stop reason, source attribution, and Stop remain not-run / approval-gated until same-thread ready preflight, sanitized output review, and operator-local env references are present.
