...
1. Kuro Live Comment Translator preview branch
   ...
   - branch stack:
     ...
     - active feature: `codex/comment-translator-youtube-credential-status-source-evidence-post-pr307-review` at `D:/V_streamer_tools/.worktrees/comment-translator-youtube-credential-status-source-evidence-post-pr307-review`
   - seed:
   ...
   - current slice target:
     - `post-PR #307 source/evidence review gate` を PR #307 (`[codex] Gate post-PR306 credential source evidence review`) merge 後の follow-up として確認する。
     - 前提 PR #307 は 2026-06-03 に `codex/comment-translator-preview` へ merge 済みで、merge commit `3b1a76a8c6b7102bc9c640e9173d016d6a5343d5` が `origin/codex/comment-translator-preview` に含まれることを確認した。
     - PR #306 merge 前提も維持し、merge commit `650917f72963bf82649ec39142e3c54f2db6bc4e` が current branch に含まれることを contract で確認する。
     ...
     - `scripts/comment-translator-youtube-credential-reference-surface-source-recheck-contract.mjs` と `lib/comment-translator-youtube-client-safe-credential-reference-source.ts` の post-PR #307 source/evidence review gate helper で、PR #307 merge 後も source と evidence が両方揃わない状態を `blocked-pr307-source-evidence-review-missing-surfaced-source-or-approval-evidence` として扱う。
