...
- Public prelaunch visual review Task 4 / Schedule Calendar pointer behavior and month preview guard
  ...
  - 追記 2026-05-17 / `schedule-review-click-detail-followup`: PR #138 / #139 merge 後の review follow-up として、PC click 後に hover preview が focus で固定されないよう `lg:group-focus-within:block` を外し、右パネルのこの日の予定一覧から重複 preview を削除。右パネル内の予定選択は PC で mobile sheet 状態を立てず、mobile layout のみ詳細 sheet を開くようにした。
  - 追加検証: `node scripts/schedule-calendar-pointer-month-guard-contract.mjs`、`node scripts/schedule-calendar-prelaunch-polish-contract.mjs`、`node scripts/schedule-calendar-storage-contract.mjs`、`node scripts/tool-handoff-contract.mjs`、`npm run lint`、`npx tsc --noEmit` は通過。in-app browser で `/tools/schedule-calendar` を `390 / 820 / 1024 / 1280 / 1366px` smoke し、body / document 横 overflow なし、console error / warn なし、focus 固定 class が出力されていないことを確認。in-app browser の virtual clipboard 制約でフォーム入力を伴う実データ click 再現は未実施。
  - 追記 2026-05-17 / `schedule-tablet-landscape-detail`: tablet landscape 相当の `1024-1279px` は click で右パネル詳細を確認できる挙動に戻し、hover-first の click 抑制は `1280px+` fine pointer の desktop range に限定した。calendar click 時も tablet two-pane では mobile sheet 状態を立てず、右パネルの schedule edit view へ切り替える。
  - 追加検証: `node scripts/schedule-calendar-pointer-month-guard-contract.mjs`、`node scripts/schedule-calendar-prelaunch-polish-contract.mjs`、`node scripts/schedule-calendar-storage-contract.mjs`、`node scripts/tool-handoff-contract.mjs`、`npm run lint`、`npx tsc --noEmit`、`git diff --check` は通過。in-app browser で `1024 / 1280 / 1366px` を smoke し、body / document 横 overflow なし、console error / warn なし、focus 固定 class なしを確認。`390 / 820px` は browser 接続上の mobile shell 表示取得が薄く、今回の対象外として実操作再現は未確認。`git diff --check` は Schedule Calendar 関連ファイルと `task.md` の LF -> CRLF warning のみ。
  - 追記 2026-05-17 / `schedule-tablet-calendar-popover`: tablet landscape 相当の `1024-1279px` では、カレンダー面の予定 click で右パネル詳細/edit view へ切り替えつつ、選択中予定の詳細 popover もカレンダー上に表示するようにした。popover の click 固定表示は `lg:block xl:hidden` に限定し、`1280px+` PC は hover-only のまま維持。
  - 追加検証: `node scripts/schedule-calendar-pointer-month-guard-contract.mjs` は RED -> GREEN を確認。`node scripts/schedule-calendar-prelaunch-polish-contract.mjs`、`node scripts/schedule-calendar-storage-contract.mjs`、`node scripts/tool-handoff-contract.mjs`、`npm run lint`、`npx tsc --noEmit`、`git diff --check` は通過。in-app browser で `1024px` は予定 click 後に選択中予定 popover が `display:block`、右パネル text 表示あり、body / document 横 overflow なし、console error / warn なしを確認。`1280px` は同じ click 後も selected popover が `xl:hidden` で非表示、focus 固定 class なし、console error / warn なしを確認。
  - 追記 2026-05-17 / `schedule-tablet-preview-dismiss`: tablet landscape の click preview は選択状態とは別の dismiss state で管理し、カレンダー面の予定 click で開き、カレンダー空白 click / 右パネル操作 / 右パネル側の予定選択で preview だけ閉じるようにした。右パネルの detail / edit view と `1280px+` PC の hover-only 方針は維持。
  - 追加検証: `node scripts/schedule-calendar-pointer-month-guard-contract.mjs` は RED -> GREEN を確認。`node scripts/schedule-calendar-prelaunch-polish-contract.mjs`、`node scripts/schedule-calendar-storage-contract.mjs`、`node scripts/tool-handoff-contract.mjs`、`npm run lint`、`npx tsc --noEmit`、`git diff --check` は通過。in-app browser で `1024px` は予定 click 後に selected preview が `display:block`、右パネルの `投稿補助` click 後に preview が消えること、body / document 横 overflow なし、console error / warn なしを確認。`1280px` は selected preview 固定表示なし、focus 固定 class なし、body / document 横 overflow なし、console error / warn なしを確認。
...
- Public final smoke / release checklist / security check
  - 結果記録 2026-05-17 / `public-final-smoke-security`: 事前 gate として `git fetch origin main --prune`、`git rev-parse HEAD origin/main`、`git status --short --branch`、`gh pr list --state open --json number,title,isDraft,headRefName,url` を確認。open PR は `[]`、`HEAD` / `origin/main` は `667a16182eb34ab5caf153bbfbc22c545de445e5` で同期済み。親 checkout の `next-env.d.ts` 差分は触らず、`origin/main` 起点の `D:/V_streamer_tools/.worktrees/public-final-smoke-security` / branch `codex/public-final-smoke-security` で実施した。
  - 変更内容: `npm audit --omit=dev` で Next.js 16.2.4 の high advisory が出たため、公開前 security blocker 候補として `next` のみ `16.2.6` へ patch 更新。新規依存、UI redesign、新機能、schema / storage 変更、外部連携追加、英語対応、ZIP 出力、X 以外の比率、複数形式一括 export は入れていない。
  - security checklist: repo 内 `.env*` は `.gitignore` の ignore rule のみで、secret / token / private key の実ファイル誤コミットなし。`materials/ideas` 内の `access_token` / `api_key` は設計メモの schema 例で実値なし。`dangerouslySetInnerHTML` / `innerHTML` / 直接 DOM injection は app code で使用なし。外部 `target="_blank"` は `rel="noreferrer"` 付き。clipboard は投稿文コピーのみで失敗時 fallback 表示あり。download は canvas export のローカル生成のみ。Blob URL / object URL は user material preview で revoke あり。file import は image MIME accept と normalize 経由、Schedule import JSON / localStorage / handoff payload は text length / target / expiry / version guard を確認。
  - release checklist: `next.config.mjs` は `output: "export"` / `trailingSlash: true` / image unoptimized、`public/_headers` は frame deny、nosniff、referrer、permissions、CSP、`frame-ancestors 'none'` を確認。`npm run build` は Next.js 16.2.6 / webpack で通過し、static export alias は `8 checked, 8 written`。worktree が親 checkout 配下にあるため Next.js の複数 lockfile root 推定 warning は出るが、`out/` の静的配信 smoke は通過。
  - 幅別確認: in-app browser で build 後の `out/` を `http://127.0.0.1:3021` として静的配信し、`/`、`/tools`、`/tools?suite=stream-workflow`、`/tools/schedule-calendar`、`/tools/thumbnail-editor`、`/tools/sns-split-image-maker`、`?preset=split-2`、`?preset=split-3`、`?preset=split-4` を `390 / 820 / 1024 / 1280 / 1366px` で確認。全45ケースで body / document 横 overflow なし、loading 固着なし、console error / warn なし。公開向け copy、利用可能 / planned 表示、主要 label、popover / menu layer の前面表示を確認。
  - 操作 smoke: Schedule Calendar は `390px` で予定作成、編集、投稿補助、`サムネを作る` handoff、SNS Split handoff copy を確認。Thumbnail Editor は Schedule handoff 由来の予定テキスト導線、export copy、`SNS分割画像へ進む` 導線、preset / canvas size / output ratio menu の `z-[120]` 前面表示と Escape close を確認。SNS Split Image Maker は Thumbnail handoff 受信、`split-4` 既定、投稿順 `split_1 -> split_2 -> split_3 -> split_4`、完成形 preview、PNG / JPEG copy 周辺を確認。
