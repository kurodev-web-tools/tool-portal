...
assert.match(hourlyDetailSource, /v_provider_receipt\.provider_kind\s+is distinct from\s+\(case[\s\S]+?end\)\s+then/i, "provider hourly authority parenthesizes its CASE expression for PostgreSQL 17 parsing");
