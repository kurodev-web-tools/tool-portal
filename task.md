...
- Current branch: `codex/pl-g3-provider-terminal-error-boundary-after-pr538`.
- Base: latest `origin/codex/comment-translator-free-public-beta-integration`.
- This branch follows up merged PR #538 with after-PR #538 contract-first provider terminal-error / provider-unavailable root-cause boundary work only. It does not rerun Start, Stop, target lookup execution, `liveChatMessages.list`, Azure/OpenAI provider execution, or UI/feed confirmation. It adds sanitized terminal error code counts and derived provider route/config labels so a later same-thread approved wrapper rerun can distinguish provider config/route/language/policy terminal classes without raw provider body/message values. Public gate state label remains unchanged / blocked, and public-release capable label remains no.
